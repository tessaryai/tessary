...
| `retrieval.documents.<N>.document.content` | Optional; required for groundedness | The retrieved text. | The text the groundedness classifier checks an answer against. |
...
The groundedness classifier reads `document.content` from the answer's own trace, or from the nearest earlier trace in the same session that retrieved anything. An answer on a `rag_answer` call site with no document content to read is not scored when its session called a tool or a retriever. See [Which answers are scored](/classifiers/groundedness#which-answers-are-scored).
