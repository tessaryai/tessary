...
| `frustration` | An OpenRouter or TypeSafe key. It is present on every project and starts switched off, because each eligible user message it scores is a call on your own key. Turning it on under **Classifiers** asks which provider to use and takes the key there. |
