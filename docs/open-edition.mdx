...
`groundedness` is in this build: its model is public (`tessaryai/groundedness-classifier-v1`, MIT) and needs a GPU, so it is listed under [Capabilities that require configuration](#capabilities-that-require-configuration) rather than here.
