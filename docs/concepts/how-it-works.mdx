...
Model calls arrive later, at triage and root-cause analysis, and they are spent per finding and per case rather than per span. The one exception is [`frustration`](/classifiers/frustration), which makes one call per message it scores. It does so only after you turn it on, only on your own OpenRouter or TypeSafe key, and only for the user messages it can score. In our testing, that shape cost 5x less than grading on 1% sampled traces.
