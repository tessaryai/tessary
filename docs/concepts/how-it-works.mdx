...
They are built to be cheap enough to run against every trace rather than a sample. None of the five that run by default costs a model call: `duration_drift`, `cost_drift` and `tool_error` are arithmetic over stored spans, and `secret_leak` and `malformed_output` are pattern and schema checks.

Model calls arrive later, at triage and root-cause analysis, and they are spent per finding and per case rather than per span. Two classifiers are exceptions, and both stay off until you turn them on. [`frustration`](/classifiers/frustration) makes one call per message it scores, only on your own OpenRouter or TypeSafe key, and only for the user messages it can score. In our testing, that shape cost 5x less than grading on 1% sampled traces. [`groundedness`](/classifiers/groundedness) runs a model on each answer it scores, but it isn't an LLM call and spends nothing on a provider key. A public model on a GPU you run, on a Mac with Apple silicon or an instance on AWS, checks whether the answer is supported by the documents the call retrieved, or by its prompt when it retrieved none.
...
Triage is the step in between. It runs an agent in a sandbox against the finding's own claim and evidence and asks one question: does the claim hold, over enough samples, carried by the evidence it cites? A `positive` ruling opens a case. A `negative` ruling closes the finding. A high-confidence secret leak and a frustration finding skip this step: the classifier files each one already ruled `positive` and opens the case itself, because its numbers are the claim.
...
<Note>
  Slack delivery is a capability of its own, and it sends nothing until the deployment is configured with a Slack app's credentials. The delivery route every organization has is a signed generic webhook, which is why **Settings → Notifications** leads with the webhook field.
</Note>
