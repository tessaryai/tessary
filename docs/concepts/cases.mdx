...
Both close every finding the case still holds open, in the same action.
...
A frustration case skips triage. The classifier rules its own finding positive and opens the case when a call site's rate of frustrated conversations rises above the rate it learned, because the numbers on the finding are the claim.

Resolving one asks what it was, and both answers restart that call site's test: the accumulator goes back to zero, and the call site learns its normal rate again from the conversations after the resolve.

| Disposition | What it does |
| --- | --- |
| **Fixed** | Restarts the test. The conversations the case cites still count as frustrated. |
| **False alarm** | Restarts the test, and every conversation cited by any finding in the case stops counting as frustrated. Their later user messages can be scored again. |
