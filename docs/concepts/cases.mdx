...
Closing on `negative` is safe because a real cause keeps firing. The finding's cause is still live, the classifier keeps refreshing it, and a recurrence sends it back through triage rather than leaving it in a queue nobody drains.
