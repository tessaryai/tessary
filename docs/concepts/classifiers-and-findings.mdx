...
`groundedness` is in this build and starts switched off on every project. It checks whether an answer is supported by the documents the call retrieved: a public model (`tessaryai/groundedness-classifier-v1`) reads the documents and the whole answer in one pass and flags the sentences they don't support. The model needs a GPU and runs outside Tessary's containers, on a Mac with Apple silicon for development or on a GPU instance on AWS for production. Turning it on under **Classifiers** gives you a prompt for your coding agent, which follows the setup file [on a Mac](https://github.com/tessaryai/tessary/blob/main/classifiers/groundedness/setup/groundedness-setup-mac.md) or [on AWS](https://github.com/tessaryai/tessary/blob/main/classifiers/groundedness/setup/groundedness-setup-aws.md).

A flagged answer is not a finding on its own. Each call site learns its normal share of traces with a flagged answer, and a finding opens when that share rises, like frustration's. Its findings go through triage, and a positive ruling opens a case. Its measured quality is on the [classifier quality page](https://github.com/tessaryai/tessary/blob/main/devdocs/reference/classifier-quality.md). [Catch answers your documents don't support](/classifiers/groundedness) covers turning it on, what it sends, what it costs, and reading the case.
...
The capability API refuses an override for either rather than accepting one and doing nothing, so enabling one returns an error instead of leaving you hunting through your traces for results that were never going to appear.
...
**A backfill files under the days it happened, not the day it was uploaded.** A per-span classifier's detections carry the time the flagged span actually ran, so a month of historical traffic checked in one afternoon still opens findings on the real days that traffic occurred, and the Classifiers page charts its volume on those days too.
