...
- **`range` is half-open, `[from, to)`,** on the dataset's own time column: `started_at` for `spans` and `tool_calls`, `created_at` for `classifier_events`, or `bucket_start` on `metric_rollups`. `describe_dataset`'s `time_column` names it per dataset.
