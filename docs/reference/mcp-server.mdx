...
| Tool | Fields removed |
| --- | --- |
| `get_finding` | `triageVerdict`, `triageAction`, `triageSummary`, `triageCitations`, `triagedAt`, `humanVerdictAt` |
| `list_findings` | The same six fields on every row |
| `get_case` | `ruling` |
...
A high-confidence secret leak and a frustration finding have no triage run to strip: each is filed already ruled, and `triageStatus` reflects that ruling. On a frustration finding or case, the `frustration` block also comes back with an empty `conversations` list, so the frustrated sessions' ids do not reach a caller through it. `get_finding_evidence` pages them as refs.

A groundedness finding goes through triage like any other, so its ruling is stripped as the table says. On a groundedness finding or case, the `groundedness` block comes back with an empty `answers` list and a null `answersNextCursor`, so the flagged answers' trace and span ids do not reach a caller through it. `get_finding_evidence` pages them as `witness` refs.
...
Tessary offers all 19 tools to every organization, so `tools/list` returns the whole catalog for any valid token.
...
| `get_case` | `id` | none | The case, its activity trail, `latest_finding_id` and `finding_count`, the classifier's summary numbers, and the root-cause report inline in `rca` once one has finished. The ruling, exemplars, and every trace and span id are redacted on this surface; page the finding's rows with `get_finding_evidence` and `latest_finding_id` |

On a frustration case, `frustration` carries the call site's learned and current rates of frustrated sessions with the session counts behind each, and the scoring settings the finding was filed under.

On a groundedness case, `groundedness` carries the call site's learned and current rates of flagged answers with the trace counts behind each, the score at which a sentence is flagged (`flagThreshold`), and the rate test's settings the finding was filed under.
...
| Tool | Required | Optional | Returns |
| --- | --- | --- | --- |
| `list_findings` | none | `status`, `call_site_id`, `detector`, `include` (`confirmed` \| `all`, default `confirmed`) | Headline finding rows without the evidence blob |
| `get_finding` | `id` | none | One finding with its parsed evidence, including the `frustration` block on a frustration finding and the `groundedness` block on a groundedness finding |
...
- **`range` is half-open, `[from, to)`,** on the dataset's own time column: `started_at` for `spans` and `tool_calls`, `subject_started_at` for `classifier_events` (the span or trace it judged, not when the classifier checked it), or `bucket_start` on `metric_rollups`. `describe_dataset`'s `time_column` names it per dataset.
