...
| `get_case` | `id` | none | The case, its activity trail, `latest_finding_id` and `finding_count`, exemplar traces, and the root-cause report inline in `rca` once one has finished. The ruling is redacted on this surface |
