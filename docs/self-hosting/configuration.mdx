...
| `TESSARY_OBSERVER_ENCODER_PROBE_INTERVAL_MS` | How often the backend checks the encoder's `/healthz`; the answer is the instance-level switch on the encoder-backed classifiers. | `60000` |

Both ship blank. Set them to run the `groundedness` classifier: start the bundled service with `docker compose --profile classify up -d` (a clone of the repository is needed for that first build, which bakes the public `tessaryai/groundedness-token-v1` model into the image; plan for about 5 GB of memory), then set `TESSARY_OBSERVER_ENCODER_URL=http://classify:8080` and `TESSARY_OBSERVER_ENCODER_API_KEY` to the same value the service is given as `CLASSIFY_API_KEY` (the compose file wires both from this one variable). With them blank, or with a service that does not answer, the backend marks the classifier unavailable on this instance rather than sweeping into nothing: it probes the service's `/healthz` on boot and every minute (`TESSARY_OBSERVER_ENCODER_PROBE_INTERVAL_MS`), `/actuator/health/encoder` reports the last answer and the reason, and the groundedness classifier appears, on for every org, within a minute of the first `200`. An org can then turn it off for itself under its capabilities, but cannot turn it on while the instance has no encoder to send to. On an Apple silicon Mac, [Groundedness on a Mac, using the GPU](/self-hosting/groundedness-on-a-mac) runs the model outside the container on the GPU instead, with the same two variables. The frustration classifier does not use this service; it needs an OpenRouter or TypeSafe key, stored under **Settings** then **Providers**. See [Encoder-based classifiers need a second service](/self-hosting/setup#encoder-based-classifiers-need-a-second-service) in Setup, and the classifier sections of [Troubleshooting](/self-hosting/troubleshooting).
