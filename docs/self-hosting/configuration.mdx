...
| Variable | Description | Default |
| --- | --- | --- |
| `TESSARY_SECRET_KEY` | AES-256-GCM key Tessary uses to seal secrets it stores, such as provider API keys. | A placeholder, published in `docker-compose.yml` |
...
<Warning>
  `TESSARY_AUTH_COOKIE_PASSWORD` is the most urgent of the three placeholders. The sealing key above needs database access to exploit; this one does not. Anyone can forge a session cookie against an instance still running the published default. Replace it before anything but your own machine can reach the instance: see [Secure the instance](/self-hosting/setup#secure-the-instance).
</Warning>
...
Who may create an account is a setting, not an environment variable: an owner or admin sets it under **Settings → Members → Sign-up policy**, and it is enforced by the backend on every path that creates an account, including a first sign-in through WorkOS. It is stored on the first organization created on the install and governs the whole instance, so a self-hoster who signs up first owns it. Anyone the policy admits later gets an organization of their own, and that organization sees the policy read-only, named with the organization that governs it.
...
The same setting is readable and writable over the API, for an owner or admin of the governing organization: `GET` and `PUT /api/orgs/{orgSlug}/signup-policy` with a body of `{"mode": "domain", "domains": ["acme.com"]}`. If you are putting the instance on a real hostname, set this before anyone else can reach it.
...
The broker and the larger backend heap that goes with it need a bigger host than the default configuration assumes. [Host sizing](/self-hosting/deployment#host-sizing) states the smallest one to run this mode on.
...
A TypeSafe key serves only the frustration classifier. An OpenRouter key serves triage, RCA, and the frustration classifier.
...
| Variable | Description | Default |
| --- | --- | --- |
| `TESSARY_OBSERVER_ENCODER_URL` | URL of the groundedness model server. | Blank |
| `TESSARY_OBSERVER_ENCODER_API_KEY` | API key for the groundedness model server. | Blank |
| `TESSARY_GROUNDEDNESS_CLASSIFIER_MODE` | `dev` scores whenever the model answers. `production` is a GPU instance that wakes on a schedule and stops when idle, so Tessary waits 30 minutes after it catches up before scoring again. | `dev` |
| `TESSARY_OBSERVER_ENCODER_PROBE_INTERVAL_MS` | How often the backend checks the model server's `/healthz`. While it doesn't answer, groundedness pauses and picks up where it left off once it does. | `60000` |

The groundedness model needs a GPU: a Mac with Apple silicon for development, or a GPU instance on AWS for production. The setup file your coding agent follows sets these on a Mac, or prints them for you to add on AWS: [on a Mac](https://github.com/tessaryai/tessary/blob/main/classifiers/groundedness/setup/groundedness-setup-mac.md) or [on AWS](https://github.com/tessaryai/tessary/blob/main/classifiers/groundedness/setup/groundedness-setup-aws.md). The frustration classifier does not use this server; it needs an OpenRouter or TypeSafe key, stored under **Settings** then **Providers**. See [Groundedness needs a GPU](/self-hosting/setup#groundedness-needs-a-gpu) in Setup.
...
| `TESSARY_OBSERVER_AGENTIC_LAUNCHER_API_KEY` | The bearer the backend presents to the sandbox launcher, and the launcher checks it against. Anyone holding the published default can drive the launcher into spawning containers on the host, so replace it before the instance holds anything real: see [Secure the instance](/self-hosting/setup#secure-the-instance). | A placeholder, published in `docker-compose.yml` |
...
CDN. Nothing else in this section is an exception to that; it exists to say precisely what the
backend sends there and fetches from there.

**What the ping sends, once on backend start and then every 6 hours:**

| Field | Example | What it is |
| --- | --- | --- |
| `contract_version` | `1` | Schema version of this payload. |
| `instance_id` | a v4 UUID | Generated once at first boot. Identifies an instance, never a person or an org. |
| `ping_seq` | `41` | How many pings this instance has sent before this one. |
| `sent_at` | an ISO-8601 UTC timestamp | When the ping was generated. |
...
| `counts` | `{"projects": 3, "spans": 1204551, "findings": 88, "cases": 12, "l1": 40210}` | Install-wide usage totals: projects, spans ingested, findings, cases, and classifier detections. One number each for the whole install, never per org or project. |
| `price_book` | `{"digest": "a5ad23f7…", "schema_max": 1}` | Which model price book this install prices from, by its sha256. |

It never carries an email address, an org or project name, trace/prompt/dataset content, a license
key, or a retained IP address.

**What the backend fetches, on the same schedule:** the model price book. It requests
`/v1/pricing/manifest.json`, a few hundred bytes naming the current book's hash, and downloads the
book itself (about 2 MB) only when that hash is one it does not already hold. It checks the download
against the hash before using it. Neither request carries a body or any identifier. New prices apply
to spans ingested afterwards; costs already recorded never change.

The full field-by-field contract lives at `devdocs/reference/telemetry-contract.md` in the repository
for anyone auditing the wire format against the running code.
...
| --- | --- | --- |
| `home.tessary.ai` | Once at backend start and every 6 hours, only while `TESSARY_TELEMETRY_ENABLED` is `true` | The heartbeat and the price book check above. Off, the name is never even resolved. |
...
| The groundedness model server you set up | Only when `TESSARY_OBSERVER_ENCODER_URL` is set: a health check every `TESSARY_OBSERVER_ENCODER_PROBE_INTERVAL_MS`, and a scoring request for each answer while the groundedness classifier is on | Your own Mac or AWS instance. Tessary sends each answer with its retrieved documents and the user's question, or with its prompt when it has no documents. The variable ships blank, so a default boot never contacts it. |

The machine that runs the groundedness model makes its own downloads, separate from Tessary's containers: `serve.py` from GitHub, the Python packages it runs on, and the model's weights from Hugging Face. See [What is sent, and to whom](/classifiers/groundedness#what-is-sent-and-to-whom).

**Turning it off costs one thing: price updates between releases.** `TESSARY_TELEMETRY_ENABLED=false`
gives Tessary no install-count or activation visibility into your deployment, and your instance stops
checking for newer model prices, so it prices from the book bundled in the release you run until you
upgrade. There is no license check, feature gate, or other in-app behavior that depends on reaching us.
...
`devdocs/reference/principles.md` § "Product & positioning" for the full argument.
...
<Note>
  `SITE_DOMAIN` is also how Tessary knows this instance is reachable from outside the host. Setting it while any of the three placeholder keys is still on its shipped value refuses the boot, naming the key. That is what makes the placeholders safe to ship: they cannot follow you onto a public host.
</Note>
