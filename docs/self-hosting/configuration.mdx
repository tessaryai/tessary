...
Both ship blank, and in the open edition they stay blank: the model weights classify-service loads are gated and not published, so the groundedness classifier is unavailable rather than merely unconfigured. The frustration classifier does not use this service; it needs an OpenRouter or TypeSafe key, stored under **Settings** then **Providers**. See [The encoder-based classifier is unavailable in the open edition](/self-hosting/setup#the-encoder-based-classifier-is-unavailable-in-the-open-edition) in Setup, and the classifier sections of [Troubleshooting](/self-hosting/troubleshooting).
