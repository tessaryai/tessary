...
## The encoder-based classifier is unavailable in the open edition

<Note>
  The groundedness classifier depends on a separate `classify-service`, and it is **not available in the open edition**. The self-hosting configuration does not start that service, its encoder URL and API key environment variables ship blank, and the model weights the service loads are gated and not published, so standing it up yourself is not a path this documentation can offer. Every other classifier runs without it; setup reaches connected traces and a first case without groundedness.
</Note>

The frustration classifier does not need that service. It sends eligible user messages to TypeSafe's Jev decision model on your own OpenRouter or TypeSafe key, so it starts switched off and asks for the key when you turn it on. [Classifiers and findings](/concepts/classifiers-and-findings#frustration-on-your-own-key) covers what it sends and when a case opens.
