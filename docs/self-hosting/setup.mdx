...
| `TESSARY_SECRET_KEY` | Seals what Tessary stores for you: provider API keys and the GitHub install-state token. Anyone who reaches your database can decrypt all of it. |
...
## Groundedness needs a GPU

The `groundedness` classifier scores answers with a model that runs on a GPU, outside Tessary's containers: on a Mac with Apple silicon for development, or, for production, on a GPU instance beside a Tessary that runs on AWS. Turning it on under **Classifiers** gives you a prompt for your coding agent, which follows one of these files:

- [Set up on a Mac](https://github.com/tessaryai/tessary/blob/main/classifiers/groundedness/setup/groundedness-setup-mac.md)
- [Deploy on AWS](https://github.com/tessaryai/tessary/blob/main/classifiers/groundedness/setup/groundedness-setup-aws.md)

A stack without a GPU runs every other classifier and reaches connected traces and a first case without groundedness.

The frustration classifier does not need a GPU. It sends eligible user messages to TypeSafe's Jev decision model on your own OpenRouter or TypeSafe key, so it starts switched off and asks for the key when you turn it on. [Watch for frustrated users](/classifiers/frustration) covers what it sends and when a case opens.
