...
## Groundedness classifier never fires

**Symptom:** Every classifier except groundedness produces findings. It never fires, even on projects with active traffic.

**Cause:** Groundedness is served by a separate `classify-service` that the default `docker-compose.yml` does not start. `.env.example` ships `TESSARY_OBSERVER_ENCODER_URL` and `TESSARY_OBSERVER_ENCODER_API_KEY` blank, so there is no encoder for it to call.

**Resolution:** None in the open edition. The model weights `classify-service` loads are gated and not published, so there is no self-host path to this classifier. This is expected behavior, not a misconfiguration: every other classifier runs without it. See the encoder section of [configuration](/self-hosting/configuration) for the variables involved.
