...
**Symptom:** Every classifier except groundedness produces findings. It never fires, even on projects whose call sites declare a retrieval shape (`rag_answer`, `summarize`, `extract`) and carry retrieved documents.

**Cause:** Groundedness is scored by a separate `classify-service` that the default `docker-compose.yml` does not start, and `TESSARY_OBSERVER_ENCODER_URL` / `TESSARY_OBSERVER_ENCODER_API_KEY` ship blank, so the sweep has nowhere to send its requests. The backend logs the failure on every tick.

**Resolution:** Start the service with `docker compose --profile classify up -d` from a clone of the repository and set the two variables as [Configuration](/self-hosting/configuration#classifier-encoder-service) describes. The classifier is enabled by default and sweeps as soon as the service answers. On a Mac, [the GPU path](/self-hosting/groundedness-on-a-mac) is the faster option. Two further conditions are by design, not faults: it only scores turns whose call site shape declares verifiable source content, and it is quiet on a turn that reached no evidence.
