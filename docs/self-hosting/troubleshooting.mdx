...
**Symptom:** The `groundedness` row under **Classifiers** reads "needs setup" or "No scores since" a time, and no groundedness findings appear, even on projects whose call sites declare a retrieval shape (`rag_answer`, `summarize`, `extract`) and carry retrieved documents.

**Cause:** Groundedness scores with a model that runs on a GPU, outside Tessary's containers. "needs setup" means it has never scored; "No scores since" means the model server stopped answering, or, on AWS, missed its hourly run. While it doesn't answer, groundedness pauses and keeps its place.

**Resolution:** Under **Classifiers**, turn it on, or open it and choose **Restart model**, then paste the prompt into your coding agent. It follows the setup file [on a Mac](https://github.com/tessaryai/tessary/blob/main/classifiers/groundedness/setup/groundedness-setup-mac.md) or [on AWS](https://github.com/tessaryai/tessary/blob/main/classifiers/groundedness/setup/groundedness-setup-aws.md). A host without a GPU can't run it. Once the model answers, groundedness scores what arrived while it was down. Two further conditions are by design, not faults: it only scores turns whose call site shape declares verifiable source content, and it is quiet on a turn that reached no evidence.
