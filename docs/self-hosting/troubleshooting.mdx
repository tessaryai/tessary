...
- An answer on a call site whose shape is not `rag_answer`, `summarize`, or `extract`, or that has no shape because no imported bundle set one. [Call-site shapes](/concepts/call-sites#call-site-shapes) says how to set it.
