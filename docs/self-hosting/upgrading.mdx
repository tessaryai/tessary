...
The matching YAML keys moved too, from `evals.*` to `tessary.*`. You only need this if you mount your
own `application.yaml` instead of setting environment variables.
...
Pin the old names and nothing moves:
...
Or rename them in place, with the stack stopped except for Postgres:

```bash
docker compose up -d postgres
docker compose exec -T postgres psql -U evals -d postgres -c 'ALTER DATABASE evals RENAME TO tessary;'
docker compose exec -T postgres psql -U evals -d postgres -c 'ALTER ROLE evals RENAME TO tessary;'
docker compose exec -T postgres psql -U tessary -d postgres -c "ALTER ROLE tessary WITH PASSWORD 'tessary';"
```

Renaming a role invalidates its stored password, so the third command is required, not optional.

## One-time: sessions, tokens and webhook headers

Three names that leave the backend changed in the same release.

- **The session cookie** is now `tessary-session`. Everyone signed in is signed out once. They sign
  back in normally; nothing else is affected.
- **New API tokens** carry a `tessary_pat_` prefix. Tokens issued before the upgrade keep their
  `evals_pat_` prefix and keep working. You do not need to reissue them.
- **Outbound alert webhooks** send `X-Tessary-Signature-256`, `X-Tessary-Event` and
  `X-Tessary-Delivery` in place of the `X-Evals-*` names. **A receiver that verifies the HMAC by
  header name rejects every delivery until you update it.** The signature format is unchanged:
  `sha256=<hex>` over the raw body. Update your receiver before you upgrade, or accept either name
  during the transition.

## One-time: Grafana alert rules

The four CPU alert rules under `observability/` changed `uid` from `evals-*` to `tessary-*`
(`tessary-cpu-high`, `tessary-cpu-steal`, `tessary-jvm-gc-thrash`, `tessary-cpu-iowait`). Grafana
treats a changed `uid` as a new rule, so provisioning creates the four new ones and leaves the old
four in place. Delete the `evals-*` rules after the upgrade, or they alert twice.
