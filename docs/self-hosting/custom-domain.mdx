...
| --- | --- | --- |
| Sign-in and sign-out landing | `https://tessary.acme-corp.com/` | `EVALS_AUTH_FRONTEND_URL`, which must then name the same host |
...
| Agentic lanes' callback origin | `https://tessary.acme-corp.com` | `EVALS_RCA_AGENTIC_MCP_BASE_URL` and `EVALS_CLASSIFIER_TRIAGE_MCP_BASE_URL`, each of which must then name the same host |
