...
cutover, as is every other workflow here bar `namespace-recheck.yml`'s quarterly cron. A PR merged on a red local check stays red on `main` until a human presses Run. Docker is required for any backend slice. Full cost model and recount commands:
