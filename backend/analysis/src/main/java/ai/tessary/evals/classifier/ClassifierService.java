...
import ai.tessary.evals.classifier.catalog.BuiltInClassifierCatalog.BuiltIn;
import ai.tessary.evals.classifier.catalog.BuiltInDetector;
import ai.tessary.evals.classifier.catalog.ClassifierSeedListener;
...
import ai.tessary.evals.classifier.metric.MetricSketch;
...
public class ClassifierService {
     ...
     * ai.tessary.evals.plan.Capability} the flag layer resolves off. The other half of {@link #availableBuiltIns}:
    ...
    public ClassifierRow get(String projectId, String id) {
        ClassifierRow row =
                signals.findById(projectId, id).orElseThrow(() -> new EvalsException(ClassifierError.NOT_FOUND, id));
        if (!reaches(row, withheldBuiltInKeys(projectId))) {
            throw new EvalsException(ClassifierError.NOT_FOUND, id);
        }
        return row;
    }
    ...
    public ClassifierRow setMode(String projectId, String id, String mode) {
        if (!ClassifierRow.Mode.DISCOVERY.equals(mode) && !ClassifierRow.Mode.TRACKING.equals(mode)) {
            throw new EvalsException(ClassifierError.INVALID_MODE, mode);
        }
    ...
    private ClassifierRow requireMetricDrift(String projectId, String id) {
        ...
        if (!BuiltInDetector.Kind.COST_DRIFT.equals(signal.detector())
            ...
            throw new EvalsException(ClassifierError.NOT_METRIC_DRIFT, id);
        }
        return signal;
    }
