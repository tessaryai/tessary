 ...
 * {@link SecretBox} (AES-256-GCM, {@code evals.secret-key}) before it touches the DB and is never
...
public class AlertChannelService {
    ...
    public AlertChannelRow require(String projectId, String id) {
        return repo.find(projectId, id).orElseThrow(() -> new EvalsException(AlertError.CHANNEL_NOT_FOUND, id));
    }
    ...
    private AlertChannelKind parseKind(String wire) {
        try {
            ...
            throw new EvalsException(AlertError.UNSUPPORTED_CHANNEL, e, wire);
        }
    }

    private String sealConfig(AlertChannelKind kind, @Nullable Map<String, Object> config) {
        if (config == null || config.isEmpty()) {
            throw new EvalsException(AlertError.INVALID_CHANNEL_CONFIG, "config is required");
        }
        if (!secretBox.isConfigured()) {
            throw new EvalsException(AlertError.MISSING_SECRET_KEY, kind.wire());
        }
        try {
            ...
            throw new EvalsException(AlertError.INVALID_CHANNEL_CONFIG, e, "could not seal config");
        }
    }
}
