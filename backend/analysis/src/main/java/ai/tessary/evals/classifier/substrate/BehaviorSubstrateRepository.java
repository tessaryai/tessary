...
public class BehaviorSubstrateRepository {
     ...
     *     disagree. Read through {@link ai.tessary.evals.storage.Timestamps#iso} — NOT {@code getString},
    ...
    private static final String SELECT_TRACE_HEAD = """
            SELECT tr.id                                          AS trace_id,
                   ...
                   -- The ENTRY POINT decides the scope, and the rollup worker already resolved it from
                   -- the root span. A trace legitimately spans several call sites (vitals groups spend
                   -- and tool errors by the span's own), so a baseline scoped to a child would model
                   ...
                   -- The trace's OWN start. In v2 this is the only clock the row has: trace carries no
                   -- created_at, deliberately, because when the row was written is not a fact about the
                   -- turn. A backfill used to compress created_at into minutes while started_at kept the
                   -- agent's real timeline, and every span-of-time rule (graduation, decay) had to read
                   -- the latter — so the sweep cursor now rides the same clock the rules do.
     ...
     * the same monotonic, gap-free cursor shape every other sweep uses, so the trace-grain sweep reuses
     * the classifier job's cursor columns verbatim.
     *
     * <p><b>{@code is_settled} replaces the old settle window.</b> The window existed because a trace row
     * appeared with its FIRST span, so sweeping it immediately truncated the trajectory — D3 then fired on
     * the step that had not arrived yet and the {@code $} pad landed in the wrong place. The flag says the
     * thing the window was guessing at: nothing has arrived since the last rollup. The cursor is
     * monotonic, so a trace skipped for being unsettled is picked up by a later sweep — with one honest
     * consequence worth stating, because it is the same one the window had: a trace that settles LATE, after
     * the cursor has passed its {@code started_at}, is not re-visited. The rollup worker's ~10s re-fire
     * cadence keeps that window small, and the alternative (a cursor on settle time) would re-scan the
     * whole corpus every time an old trace received a straggler.
     *
     * <p>The cursor now rides {@code started_at} rather than the retired {@code created_at}, so it is on
     * the same clock as {@code counted_through_at}. In-flight cursors from the previous release are on the
     * old clock and are cleared by this release's migration; a stale one would only skip or repeat work,
     * never corrupt a profile, because admission is idempotent.
     */
     ...
     * {@code (upToTs, upToId)}, oldest first: the catch-up lane behind the project-wide cursor (#1247).
     ...
     * than the ones named: the scopes the catch-up lane has never opened (#1247). Bounded below by time so
     ...
     * re-introducing exactly the cross-call-site masking the per-call-site grain exists to remove. It
     * also ran the same scan once per epoch per fit tick for byte-identical input.
     */
    ...
    public Optional<TailWindow> tailWindow(String projectId, String callSiteId, int limit) {
        return jdbc.sql("SELECT count(*) AS traces, min(t.event_at) AS boundary FROM (" + SELECT_TRACE_HEAD + """
                ...
                .query((rs, n) ->
                        new TailAgg(rs.getLong("traces"), ai.tessary.evals.storage.Timestamps.iso(rs, "boundary")))
                .single()
                .toWindow();
    }
     ...
     *
     * <p><b>Ordering is {@code (started_at, id)} and nothing else.</b> v1 led with {@code observation.seq},
     * a producer-supplied counter that is NULL on every OTLP-ingested span (134k+ of them in production),
     * then fell through to {@code created_at}, which is identical across an ingest batch, and finally to a
     * ULID tiebreak — so the trajectory's order was decided by an id for most real traffic. v2 makes
     * {@code started_at} NOT NULL, which is the column that actually orders an agent's actions, and the
     * span id breaks its ties deterministically.
     */
    public List<TraceAction> actionsForTraces(String projectId, List<String> traceIds) {
        ...
        return jdbc.sql("""
            SELECT s.trace_id AS trace_id,
                   s.kind     AS kind,
                   -- The alphabet keys on the TOOL's name, never on the span's. A producer following
                   -- the gen_ai convention names the span `execute_tool verify_member` and puts the
                   -- bare name on `gen_ai.tool.name`, so reading `s.name` put the operation verb inside
                   -- every tool symbol. Three sources, in the ingestion contract's own precedence:
                   --   1. tool_call.name  — the normalized column the contract names as the target
                   --   2. the raw attribute
                   --   3. the span name   — a producer that sets neither
                   -- The attribute now comes from span_payload rather than a column on the span: it is a
                   -- bounded, keyed 1:1 join over one batch of traces, the same documented exception
                   -- SubstrateReadRepository's detector projections take.
            ...
            -- Exactly one row per span. `tool_call` is unique only on
            -- (span_id, source_external_id) and that column is nullable, so a re-ingest can
            -- leave two rows and a plain join would duplicate the action — inflating the counts and
            -- the fan-out width. Ordered like every other span read in this slice.
            LEFT JOIN LATERAL (
                SELECT tc2.name
                  FROM tool_call tc2
     ...
     * never carried one, which is fine — the triage agent falls back to the default branch.
     */
