 ...
 *
 * <p>Authority: see {@code devdocs/reference/trace-schema.md} for the full schema + the OpenInference
...
public final class GenAiAttributes {
    ...
    // Read by the ingest pricer: a cost the producer computed is stored verbatim and never repriced,
    // since it's a fact about what the call was billed at. No mapper change carries these; the OTLP
    // mapper already flattens every span attribute into RawEntry.metadata, so naming them here is what
    // makes them readable rather than incidental.
    ...
    // span to an Environment row. The Environment concept is gone, so nothing structural reads it any
    // more. The attribute is still accepted; a producer that ships it is not rejected, and it survives
    // as an ordinary attribute in the span's payload, it just no longer scopes anything. Do not re-add
    // a structural read without a scope to put it in.
    ...
    // not yet populated into storage. See devdocs/reference/trace-schema.md §Overlay.
    ...
    // semconv; do not treat them as canonical. See devdocs/reference/trace-schema.md §Flattened message encoding.
     ...
     * string had it rewritten to {@code [REDACTED_PHONE]} because ten digits reads as a phone number,
     * and the span then fell through to an inferred price: $0.000150 recorded for a call
