 ...
 *
 * <p>The two eval units are named for the layer they bill: cheap per-event filtering that runs on
 * everything, and the expensive escalation applied only to what the filter flagged. That is the
 * distinction a customer is charged on, so it is the distinction the unit names state.
 *
 * <p>{@link #STORAGE} is a level snapshot, not an event-stream count, and is produced only when
 * {@code tessary.metering.storage-enabled} is set: its billable basis (bytes vs row-count vs
 * retention-days) is an open billing product decision, so it stays gated until a deployment
 * ratifies the basis and opts in. The v1 basis is ingested span rows at rest.
 */
public enum UsageUnit {
    ...
    L1_EVALS("l1_evals"),
    /** LLM tokens consumed by grading. Retired; see {@link #L2_EVALS}. */
    LLM_TOKENS("llm_tokens"),
    /**
     * USD cost of grading paid on the platform's own credential, in micro-dollars (1e-6 USD).
     * Retired; see {@link #L2_EVALS}.
     *
     * <p>Micro-dollars, not dollars, because {@code metric_rollup} values move through this layer as
     * {@code long}: money must not be truncated to whole dollars, and integer micros sum exactly with
     * no floating-point drift across a month. Divide by 1e6 at the display boundary.
     *
     * <p>Live per-lane, per-model LLM spend reads the {@code llm_call} ledger through
     * {@code LlmUsageQueryRepository}, a separate system this unit never fed.
     */
    LLM_COST_MICRO_USD_PLATFORM("llm_cost_micro_usd_platform"),
    /** As {@link #LLM_COST_MICRO_USD_PLATFORM}, for a customer's own credential. Retired. */
    ...
    /**
     * The units the metering worker actually produces: everything but the reserved {@link #STORAGE}
     * and the four retired units. A retired unit is still a valid {@link #fromWire} value (history
