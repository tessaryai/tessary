...
import ai.tessary.evals.storage.TraceV2Repository.Claim;
import ai.tessary.evals.storage.TraceV2Repository.Recomputed;
...
public class TraceRollupWorker {
    ...
    @Scheduled(
            fixedDelayString = "${evals.ingest.substrate.rollup-interval-ms:1000}",
            initialDelayString = "${evals.ingest.substrate.rollup-interval-ms:1000}")
    public void tick() {
        try {
            runOnce();
