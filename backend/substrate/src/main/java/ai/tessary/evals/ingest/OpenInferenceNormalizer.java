 ...
 * re-emits via {@link ai.tessary.evals.ingest.export.TraceSpanMapper} to the same {@code gen_ai.*} span.
 *
...
public final class OpenInferenceNormalizer {
     ...
     * {@code [{role, content}]} arrays (the role-tagged shape {@link ai.tessary.evals.ingest.export.TraceSpanMapper}
     ...
     * {@code [{role, content}]} JSON string — the shape {@link ai.tessary.evals.ingest.export.TraceSpanMapper}
     ...
     * so {@link ai.tessary.evals.ingest.export.TraceSpanMapper#toSpan} re-emits them as structured attributes (the
