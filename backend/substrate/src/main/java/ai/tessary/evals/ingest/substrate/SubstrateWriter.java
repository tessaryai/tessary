...
import ai.tessary.evals.ingest.spool.IngestSpool;
import ai.tessary.evals.ingest.substrate.v2.SpanBatchWriter;
 ...
 * (one in {@code memory} mode, {@code evals.ingest.spool.kafka.consumers} in {@code kafka} mode) run the {@link SpanBatchWriter} and writes {@code session → trace → span} off the hot path. This
 ...
 * {@code evals.ingest.substrate.queue-max-bytes} before queueing it and releases exactly that
 ...
 * {@code evals.ingest.substrate.max-attempts} with linear backoff; a replay of a partially-written
