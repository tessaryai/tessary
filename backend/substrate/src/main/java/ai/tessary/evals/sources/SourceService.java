...
import ai.tessary.evals.sources.SourceDtos.CreateSourceRequest;
...
public class SourceService {
    ...
    public static final String FAKE_PROVIDER = ai.tessary.evals.ingest.upstream.FakeUpstreamSource.PROVIDER;

    /**
     * Provider tag for the synthetic source backing data ingested through the OTLP receiver.
     * It is never network-fetched: its {@link ai.tessary.evals.ingest.IngestionSource} reads the project's
    ...
    public SourceRow get(String projectId, String id) {
        return sources.findById(projectId, id).orElseThrow(() -> new EvalsException(IngestError.SOURCE_NOT_FOUND, id));
    }
    ...
    public SourceRow create(String projectId, CreateSourceRequest req) {
        if (!SUPPORTED.contains(req.provider())) {
            throw new EvalsException(IngestError.UNSUPPORTED_PROVIDER, req.provider());
        }
        ...
        if (sources.findByName(projectId, req.name()).isPresent()) {
            throw new EvalsException(IngestError.DUPLICATE_NAME, req.name());
        }
        boolean network = !NON_NETWORK_PROVIDERS.contains(req.provider());
        String enc;
        if (network) {
            if (!secretBox.isConfigured()) {
                throw new EvalsException(IngestError.MISSING_SECRET_KEY);
            }
     ...
     * This source is never network-fetched (see {@link ai.tessary.evals.ingest.SourceFactory});
    ...
    public void delete(String projectId, String id) {
        if (!sources.deleteById(projectId, id)) {
            throw new EvalsException(IngestError.SOURCE_NOT_FOUND, id);
        }
    }
