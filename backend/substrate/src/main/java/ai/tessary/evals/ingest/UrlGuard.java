...
public final class UrlGuard {
    ...
    public static URI requirePublicHttp(String baseUrl) {
        if (baseUrl == null || baseUrl.isBlank()) {
            throw new EvalsException(IngestError.INVALID_BASE_URL, "must be non-empty");
        }
        URI uri;
        try {
            uri = new URI(baseUrl);
        } catch (URISyntaxException e) {
            throw new EvalsException(IngestError.INVALID_BASE_URL, e, "malformed URI");
        }
        String scheme = uri.getScheme();
        if (scheme == null || !("http".equalsIgnoreCase(scheme) || "https".equalsIgnoreCase(scheme))) {
            throw new EvalsException(IngestError.INVALID_BASE_URL, "scheme must be http or https");
        }
        String host = uri.getHost();
        if (host == null || host.isBlank()) {
            throw new EvalsException(IngestError.INVALID_BASE_URL, "host missing");
        }
        ...
        if (uri.getUserInfo() != null) {
            throw new EvalsException(IngestError.INVALID_BASE_URL, "userinfo not allowed");
        }
        InetAddress[] addrs;
        try {
            addrs = InetAddress.getAllByName(host);
        } catch (UnknownHostException e) {
            throw new EvalsException(IngestError.INVALID_BASE_URL, e, "host did not resolve");
        }
        for (InetAddress a : addrs) {
            if (a.isLoopbackAddress()
                    ...
                    || isUniqueLocalIpv6(a)
                    || isReservedIpv4(a)) {
                throw new EvalsException(IngestError.INVALID_BASE_URL, "host resolves to a private/internal address");
            }
        }
        return uri;
    }
