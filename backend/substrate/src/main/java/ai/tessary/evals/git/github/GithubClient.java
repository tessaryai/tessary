...
import ai.tessary.evals.git.GitProviderClient;
...
public class GithubClient implements GitProviderClient {
    ...
    public List<RepoFile> getTreeFiles(GitIntegrationRow integ, String sha, String pathPrefix) {
        ...
        if (tree.path("truncated").asBoolean(false)) {
            ...
            throw new EvalsException(GitError.PROVIDER_CALL_FAILED, LABEL, "tree truncated for " + prefix);
        }
        ...
        try {
            ...
            throw new EvalsException(GitError.PROVIDER_CALL_FAILED, e, LABEL, "interrupted fetching bundle");
        } catch (ExecutionException e) {
            // Surface the worker's original EvalsException (its own stack is intact).
            if (e.getCause() instanceof EvalsException ee) throw ee; // NOPMD PreserveStackTrace
            throw new EvalsException(GitError.PROVIDER_CALL_FAILED, e, LABEL, "bundle blob fetch failed");
        }
    }
    ...
    private JsonNode patch(GitIntegrationRow integ, String path, JsonNode body) {
        ...
        try {
            ...
            throw new EvalsException(GitError.PROVIDER_CALL_FAILED, e, LABEL, "request encode failed");
        }
    ...
    private HttpResponse<String> exchange(HttpRequest req) {
        ...
        try {
            ...
            throw new EvalsException(GitError.PROVIDER_CALL_FAILED, e, LABEL, "network error");
        }
    }
    ...
    private JsonNode validateAndParse(HttpResponse<String> res) {
        ...
        if (status / 100 != 2) {
            ...
            throw new EvalsException(GitError.PROVIDER_CALL_FAILED, LABEL, "HTTP " + status);
        }
        try {
            return mapper.readTree(res.body());
        } catch (Exception e) {
            throw new EvalsException(GitError.PROVIDER_CALL_FAILED, e, LABEL, "non-JSON response");
        }
    }
