 ...
 *
 * <p><b>DB row wins over env when present.</b> On {@link ApplicationReadyEvent}, a stored row
 * overwrites whatever {@code EVALS_GIT_GITHUB_*} handed {@link GithubAppProperties} at Spring-binding
 * time. This is deliberate: a self-hoster who ran the wizard has a DB row and no env vars, and env
 * vars stay authoritative for a hosted/paid deployment that never runs the wizard (no row, nothing to
 * overwrite) — so an operator who sets env vars WITHOUT clearing a leftover DB row will find the row
 * keeps winning, same as it did the first time. That's the same "env is the loser once a row exists"
 * rule apply()'ed twice, not two different rules — but it is a real surprise to flag if this ever
 * gets tripped over in an incident.
 *
 ...
 * {@link ai.tessary.evals.synth.SignalResolver} in a different module entirely) from ever observing
...
public class GithubAppConfigService {
    ...
    public void loadOnStartup() {
        ...
        if (enc.isEmpty()) {
            return; // no wizard has ever run — env (or nothing) stays authoritative, as before #860.
        }
        if (!secretBox.isConfigured()) {
            // A row exists but we can't open it (no EVALS_SECRET_KEY). Fail closed to the pre-#860
            // behavior (env-or-unconfigured) rather than crash the whole app on a missing key that
            // may be perfectly fine for a deployment that never captured a BYO App.
    ...
    /** True once an App is live — from a prior wizard run (DB row) or from {@code EVALS_GIT_GITHUB_*} env vars. */
    ...
    private String seal(CapturedApp app) {
        if (!secretBox.isConfigured()) {
            throw new EvalsException(GitError.MISSING_APP_CONFIG, "github");
        }
        try {
