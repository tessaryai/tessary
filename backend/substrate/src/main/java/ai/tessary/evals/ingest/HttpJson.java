...
import ai.tessary.evals.open.errors.UpstreamRateLimitedException;
...
public final class HttpJson {
    ...
    private JsonNode send(HttpRequest req) {
        EvalsException lastTransient = null;
        ...
        throw lastTransient != null
                ? lastTransient
                : new EvalsException(IngestError.UPSTREAM_FAILED, providerLabel, "no response");
    }
    ...
    private record Attempt(
            ...
            @Nullable EvalsException fatal,
            @Nullable EvalsException transientError,
            long retryAfterMs) {
        static Attempt ok(JsonNode body) {
            return new Attempt(body, null, null, -1);
        }

        static Attempt fatal(EvalsException e) {
            return new Attempt(null, e, null, -1);
        }

        static Attempt transientFailure(EvalsException e, long retryAfterMs) {
    ...
    private Attempt attemptOnce(HttpRequest req) {
        ...
        try {
            ...
            return Attempt.transientFailure(
                    new EvalsException(IngestError.UPSTREAM_FAILED, e, providerLabel, "network error"), -1);
        } catch (InterruptedException e) {
            Thread.currentThread().interrupt();
            // Interrupted: do not retry — the caller wants to stop.
            return Attempt.fatal(new EvalsException(IngestError.UPSTREAM_FAILED, e, providerLabel, "network error"));
        }
        int status = res.statusCode();
        if (status == 401 || status == 403) {
            return Attempt.fatal(new EvalsException(IngestError.UPSTREAM_AUTH_FAILED, providerLabel));
        }
        if (status == 429) {
            long retryAfter =
        ...
        if (status >= 500) {
            ...
            return Attempt.transientFailure(
                    new EvalsException(IngestError.UPSTREAM_FAILED, providerLabel, "HTTP " + status), -1);
        }
        if (status / 100 != 2) {
            ...
            return Attempt.fatal(new EvalsException(IngestError.UPSTREAM_FAILED, providerLabel, "HTTP " + status));
        }
        try {
            return Attempt.ok(mapper.readTree(res.body()));
        } catch (Exception e) {
            return Attempt.fatal(
                    new EvalsException(IngestError.UPSTREAM_FAILED, e, providerLabel, "non-JSON response"));
        }
    }
