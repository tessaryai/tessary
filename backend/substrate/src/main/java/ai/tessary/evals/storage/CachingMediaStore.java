 ...
 * {@code evals.media.cache.max-bytes}. A single object larger than the whole budget is served but
...
public class CachingMediaStore implements MediaStore {
    ...
    public CachingMediaStore(
            PostgresMediaStore delegate, @Value("${evals.media.cache.max-bytes:0}") long configuredMaxBytes) {
