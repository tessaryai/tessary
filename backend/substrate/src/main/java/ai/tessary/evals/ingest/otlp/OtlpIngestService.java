 ...
 * {@code EvalsException(QUOTA_EXCEEDED)} — a thrown fault reads more honestly than a third state wedged into
...
public class OtlpIngestService {
     ...
     * @throws ai.tessary.evals.open.errors.EvalsException {@code 402 QUOTA_EXCEEDED} when the org has reached
    ...
    public IngestOutcome ingest(String projectId, ExportTraceServiceRequest request) {
        ...
        if (rejected > 0) {
            response.setPartialSuccess(ExportTracePartialSuccess.newBuilder()
                    ...
                    .setErrorMessage("span batch exceeded evals.ingest.otlp.max-spans-per-request=" + max)
                    .build());
        }
