...
import ai.tessary.evals.config.MeteringProperties;
...
import ai.tessary.evals.usage.MetricRollupJobRepository;
import ai.tessary.evals.usage.MetricRollupJobRow;
 ...
 * over the bucket window. It is gated behind {@code evals.metering.storage-enabled} (default off) because
...
public class MeteringWorker {
    ...
    @Scheduled(fixedDelayString = "${evals.metering.heartbeat-ms:300000}")
    public void tick() {
     ...
     *
     * <p>Gated behind {@code evals.metering.storage-enabled} (default off): the billable BASIS (rows vs bytes
