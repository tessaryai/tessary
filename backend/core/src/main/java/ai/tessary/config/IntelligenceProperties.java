...
/**
 * Deployment-level intelligence-mode governance, bound from {@code tessary.intelligence-mode.*}.
 ...
 * <em>regardless</em> of {@code tessary.priors.enabled} — so a misconfigured or accidentally-enabled
 * {@code tessary.priors.*} can never cause cross-tenant pooling on a single-tenant deployment. The
 * flag wins over every per-org consent row.
 *
 * <p><b>Why a second flag on top of {@code tessary.priors.enabled}.</b> {@code tessary.priors.enabled}
 ...
 */
@Component
@ConfigurationProperties(prefix = "tessary.intelligence-mode")
public class IntelligenceProperties {

    /**
     * Tenancy mode for intelligence. {@code true} (default) → <b>single-tenant</b>: no tenant's
     * data is ever pooled or read across the cross-customer boundary, whatever {@code tessary.priors.*}
     * or per-org consent say. {@code false} → the governed cross-customer pooling pipeline is allowed
     * to run (still gated, separately, by {@code tessary.priors.enabled} + per-org consent +
