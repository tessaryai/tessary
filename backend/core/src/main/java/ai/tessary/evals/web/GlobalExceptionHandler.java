...
import ai.tessary.evals.open.errors.ErrorCode;
...
public class GlobalExceptionHandler {
    ...
    @ExceptionHandler(EvalsException.class)
    public ResponseEntity<ApiResponse<Void>> handleEvals(EvalsException ex) {
        HttpStatus status = ex.error().status();
        if (status.is5xxServerError()) {
            // 5xx EvalsExceptions (upstream failures, internal errors) are otherwise
