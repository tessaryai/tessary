 ...
 * <p>Master key from {@code evals.secret-key} (base64 of 32 raw bytes). If absent,
...
public final class SecretBox {
    ...
    public SecretBox(EvalsProperties props) {
        ...
        try {
            ...
            throw new IllegalStateException("evals.secret-key is not valid base64", e);
        }
        if (raw.length != 32) {
            throw new IllegalStateException("evals.secret-key must decode to 32 bytes (AES-256); got " + raw.length);
        }
        this.key = raw;
    ...
    private byte[] requireKey() {
        if (key == null) {
            throw new IllegalStateException(
                    "evals.secret-key is not configured; set EVALS_SECRET_KEY to base64-encoded 32 bytes");
        }
        return key;
    }
}
