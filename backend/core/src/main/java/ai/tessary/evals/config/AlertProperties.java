...
/**
 * Tuning for the async alerting engine, bound from {@code evals.alert.*}.
 ...
 * from {@code evals.alert.heartbeat-ms}; {@link #defaultDigestCron} /
 * {@link #cronZone} are fallback server defaults used when a per-row cron is absent.
 */
@Component
@ConfigurationProperties(prefix = "evals.alert")
