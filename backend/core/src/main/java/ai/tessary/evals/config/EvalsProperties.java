 ...
 * with {@code EVALS_} (Spring relaxed binding).
 *
 * <p>Pipeline content lives in Postgres; the DB is the runtime source of truth.</p>
 *
 * <p>Database connection (required): {@code EVALS_JDBC_URL} + {@code EVALS_DB_USERNAME}
 * + {@code EVALS_DB_PASSWORD}. Set by docker-compose in dev/prod and injected from the
 * pgvector Testcontainers instance in tests; a blank URL fails fast (see
 * {@code DataSourceConfig}).</p>
 */
@Component
@ConfigurationProperties(prefix = "evals")
public class EvalsProperties {
     ...
     * EVALS_SITE_DOMAIN} (see {@code application.yaml}) because that is the one name the compose
