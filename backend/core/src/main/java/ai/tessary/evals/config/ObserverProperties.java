...
/**
 * The agentic-sandbox and encoder configuration, bound from {@code evals.observer.*}.
 *
 ...
 * conformance encoder and the metric detectors call. Renaming {@code evals.observer.*} would mean
 ...
 * falls {@code EVALS_RCA_AGENTIC_LAUNCHER_API_KEY} back to the observer-named key for the same reason.
 *
 ...
 */
@Component
@ConfigurationProperties(prefix = "evals.observer")
public class ObserverProperties {
     ...
     * {@code evals.observer.encoder.*}: the standalone classify-service, running on ECS
     ...
     * {@code evals.observer.agentic.*}. Only consulted when that analyzer is active.
     */
    public static class Agentic {
        ...
        // with no seeded rows (RCA and triage share it; grader generation has evals.synth.agentic-model).
