...
/**
 * Postgres {@link DataSource}. Builds a HikariCP pool against {@code EVALS_JDBC_URL} (with
 * {@code EVALS_DB_USERNAME}/{@code EVALS_DB_PASSWORD}). This is the single path for every
 * environment:
 *
 * <ul>
 ...
 *       {@code src/test/resources/META-INF/spring.factories}) injects {@code EVALS_JDBC_URL}
 *       into every test context, pointed at a freshly created database.</li>
 * </ul>
 *
 * <p>There is no embedded fallback: a blank {@code EVALS_JDBC_URL} is a configuration error and
 * fails fast.</p>
 */
@Configuration
public class DataSourceConfig {
    ...
    @Bean
    public DataSource dataSource(EvalsProperties props) {
        String jdbcUrl = props.getJdbcUrl();
        if (jdbcUrl == null || jdbcUrl.isBlank()) {
            throw new IllegalStateException("EVALS_JDBC_URL is required. In dev/prod it is set by docker-compose; "
