...
/**
 * Cross-customer priors governance, bound from {@code evals.priors.*}. <b>Disabled by
 ...
 * ({@code evals.priors.enabled=true}) <em>and</em> each contributing org opts in individually
 ...
 */
@Component
@ConfigurationProperties(prefix = "evals.priors")
public class PriorsProperties {

    /**
     * Master switch. {@code false} (default) → fully single-tenant: the
     * {@link ai.tessary.evals.priors.PriorsService} refuses to publish any pooled prior and
    ...
    void validate() {
        if (minCohort < 2) {
            throw new IllegalStateException("evals.priors.min-cohort must be >= 2 (a cohort of one leaks a single "
                    + "tenant's aggregate across the boundary); got " + minCohort);
        }
        if (epsilon <= 0.0 || Double.isNaN(epsilon)) {
            throw new IllegalStateException(
                    "evals.priors.epsilon must be > 0 (it is the DP privacy budget); got " + epsilon);
        }
        if (sensitivity <= 0.0 || Double.isNaN(sensitivity)) {
            throw new IllegalStateException(
                    "evals.priors.sensitivity must be > 0 (it scales the DP noise); got " + sensitivity);
        }
    }
