 ...
 */
@Component
@ConfigurationProperties(prefix = "evals.ingest.spool")
public class IngestSpoolProperties {
        ...
        public void setReplicationFactor(short v) {
            if (v < 1) {
                throw new IllegalArgumentException(
                        "evals.ingest.spool.kafka.replication-factor must be at least 1, not " + v);
            }
    ...
    public void setMode(String mode) {
        ...
        if (!MODES.contains(m)) {
            throw new IllegalArgumentException(
                    "evals.ingest.spool.mode must be one of " + MODES + ", not '" + mode + "'");
        }
        this.mode = m;
    }
