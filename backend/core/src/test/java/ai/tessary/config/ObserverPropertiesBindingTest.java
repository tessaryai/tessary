 ...
 * {@code TESSARY_OBSERVER_ENCODER_URL} / {@code TESSARY_OBSERVER_ENCODER_API_KEY} must reach
 * {@code tessary.observer.encoder.*} through Spring's relaxed env-var binding. There is no
    ...
    void encoderEnvVarsBindToEncoderProperties() {
        ...
        env.getPropertySources()
                .addFirst(new SystemEnvironmentPropertySource(
                        ...
                        Map.of(
                                "TESSARY_OBSERVER_ENCODER_URL", "http://classify.test.internal:8080",
                                "TESSARY_OBSERVER_ENCODER_API_KEY", "test-bearer")));

        ObserverProperties props = Binder.get(env)
                .bind("tessary.observer", Bindable.ofInstance(new ObserverProperties()))
                .get();
