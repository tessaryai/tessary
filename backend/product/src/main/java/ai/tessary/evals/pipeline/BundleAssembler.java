...
import ai.tessary.evals.model.Capability;
import ai.tessary.evals.model.Chain;
import ai.tessary.evals.model.FailureMode;
import ai.tessary.evals.model.ImplicitInvariant;
import ai.tessary.evals.model.InvariantCoverage;
import ai.tessary.evals.model.Pack;
import ai.tessary.evals.model.Pipeline;
import ai.tessary.evals.model.ProductProfile;
import ai.tessary.evals.model.Progress;
import ai.tessary.evals.model.Runtime;
import ai.tessary.evals.model.TaxonomyNode;
...
public class BundleAssembler {
    ...
    public AssembledBundle assemble(List<NamedBody> files) {
        ...
        for (NamedBody f : files) {
            ...
            try {
                ...
                throw new EvalsException(PipelineError.MALFORMED_YAML, e, relPath);
            }
        }

        if (!sawAnyShard) {
            throw new EvalsException(PipelineError.NO_SHARDS);
        }
        if (sc.meta == null) {
            throw new EvalsException(PipelineError.MISSING_META);
        }
    ...
    private void applyShard(ShardCollector sc, Shard shard, String relPath, String body) throws IOException {
        switch (shard) {
            case META -> {
                if (sc.meta != null) {
                    throw new EvalsException(PipelineError.DUPLICATE_FILE, "pipeline/meta.yaml");
                }
                sc.meta = yamlMapper.readTree(body);
            }
            case PACKS -> {
                if (sc.packsSeen) {
                    throw new EvalsException(PipelineError.DUPLICATE_FILE, "pipeline/packs.yaml");
                }
                sc.packsSeen = true;
            ...
            case PRODUCT_PROFILE -> {
                if (sc.productProfileSeen) {
                    throw new EvalsException(PipelineError.DUPLICATE_FILE, "pipeline/product_profile.yaml");
                }
            ...
            case INVARIANTS -> {
                if (sc.invariantsSeen) {
                    throw new EvalsException(PipelineError.DUPLICATE_FILE, "pipeline/invariants.yaml");
                }
            ...
            case CHAINS -> {
                if (sc.chainsSeen) {
                    throw new EvalsException(PipelineError.DUPLICATE_FILE, "pipeline/chains.yaml");
                }
                sc.chainsSeen = true;
            ...
            case TAXONOMY -> {
                if (sc.taxonomySeen) {
                    throw new EvalsException(PipelineError.DUPLICATE_FILE, "pipeline/taxonomy.yaml");
                }
            ...
            case CAPABILITIES -> {
                if (sc.capabilitiesSeen) {
                    throw new EvalsException(PipelineError.DUPLICATE_FILE, "pipeline/capabilities.yaml");
                }
            ...
            case CALL_SITE -> {
                ...
                if (cs.id() == null || cs.id().isBlank()) {
                    throw new EvalsException(PipelineError.MISSING_ID, relPath);
                }
                sc.callSites.add(cs);
            }
            case FAILURE_MODES -> {
    ...
    private <T> List<T> readList(JsonNode node, Class<T> elementType) throws IOException {
        ...
        if (!node.isArray()) {
            throw new EvalsException(PipelineError.EXPECTED_YAML_LIST, node.getNodeType());
        }
    ...
    private final class ShardCollector {
        ...
        private @Nullable Runtime readRuntime(JsonNode node) {
            ...
            try {
                ...
                throw new EvalsException(PipelineError.MALFORMED_META_FIELD, e, "runtime");
            }
        }
        ...
        private @Nullable Progress readProgress(JsonNode node) {
            ...
            try {
                ...
                throw new EvalsException(PipelineError.MALFORMED_META_FIELD, e, "progress");
            }
        }
