...
import ai.tessary.evals.llm.StructuredOutput.StructuredOutputException;
import ai.tessary.evals.llmspi.ServiceTier;
import ai.tessary.evals.open.errors.EvalsException;
import ai.tessary.evals.open.errors.JudgeError;
...
public class LlmCaller {
    ...
    /**
     * Prompt-cache TTL in millis, derived from {@code evals.grader.cache.ttl} (default 5m).
     ...
     * cache hit-rate/amortization telemetry and {@code evals.judge.duplicate_verdict}.
     */
    ...
    public LlmCaller(
        ...
        this.pricer = pricer;
        this.pacer = pacer;
        this.tracer = openTelemetry.getTracer("ai.tessary.evals.llm");
        this.mapper = mapper;
        this.cacheTtlMs = parseCacheTtlMs(cacheTtl);
        this.structuredToolMissCounter = openTelemetry
                .getMeter("ai.tessary.evals.llm")
                .counterBuilder("evals.llm.structured_output.tool_miss")
        ...
        this.cacheLikelyExpiryCounter = openTelemetry
                .getMeter("ai.tessary.evals.llm")
                .counterBuilder("evals.cache.likely_expiry")
    ...
    private static void recordRequest(
        ...
        if (projectId != null) {
            span.setAttribute("evals.project.id", projectId);
    ...
    private PlatformCallPricer.@Nullable PricedCall recordResponse(
        ...
        span.setAttribute("evals.latency_ms", completion.elapsedMs());
        ...
        for (int attempt = 0; attempt <= maxRetries; attempt++) {
            if (paced) {
                try {
                    ...
                    throw new EvalsException(JudgeError.UPSTREAM_FAILED, ie, "interrupted while pacing llm call");
                }
            }
