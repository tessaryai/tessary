...
import ai.tessary.evals.config.IntelligenceProperties;
 ...
 * and {@link #derive} all no-op — <em>regardless</em> of {@code evals.priors.enabled} or any
...
public class PriorsService {
    ...
    void logTenancyMode() {
        if (intelligence.isSingleTenant()) {
            log.info("priors: SINGLE-TENANT intelligence mode is ON (evals.intelligence-mode.single-tenant=true) "
                    + "— cross-customer pooling is hard-disabled; no contribution is pooled and no prior "
                    + "crosses a tenant boundary regardless of evals.priors.enabled");
        } else {
            log.warn(
                    "priors: single-tenant intelligence mode is OFF (evals.intelligence-mode.single-tenant=false) "
                            + "— governed cross-customer pooling MAY run (still gated by evals.priors.enabled={}, "
     ...
     * is on — the top-level single-tenant gate that wins over {@code evals.priors.enabled} and every consent
