...
package ai.tessary.evals.sandbox;
...
public final class AgentSpanTelemetry {
     ...
     * ai.tessary.evals.llm.LlmCaller} uses, so the trace shows WHAT was asked and produced instead of a
            ...
            for (JsonNode turn : coalesceTurns(turns)) {
                ...
                try {
                    ...
                    if (tools.isArray() && !tools.isEmpty()) {
                        turnSpan.setAttribute("evals.agent.tools", tools.toString());
                    }
