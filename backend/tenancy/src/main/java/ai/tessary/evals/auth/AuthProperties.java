 ...
 * {@link AuthController}/{@link ai.tessary.evals.auth.link.DeviceLinkController} redirect the
 * browser, regardless of which {@link AuthProvider} is configured.
 *
 * <p><b>{@code EVALS_AUTH_DISABLED} ({@code evals.auth.disabled}), default <b>false</b>.</b>
 ...
 */
@Component
@ConfigurationProperties(prefix = "evals.auth")
public class AuthProperties {
     ...
     * other eight deploy switches are all {@code *.enabled}. An {@code evals.auth.enabled} spelt
     ...
     * Naming the dangerous state is what makes it greppable -- {@code EVALS_AUTH_DISABLED=true}
     ...
     */
    private String cookieName = "evals-session";
    ...
    /** Base64 32-byte AES key ({@code EVALS_AUTH_COOKIE_PASSWORD}) {@link SessionCipher} seals the
