 ...
 * records why #857 refused to make a blank {@code evals.rca.agentic.mcp-base-url} fatal at bean
...
public class PlaceholderSecretGuard {
    ...
    public PlaceholderSecretGuard(EvalsProperties evals, AuthProperties auth, RcaProperties rca) {
        this.evals = evals;
        this.auth = auth;
        this.rca = rca;
    }
    ...
    void verify() {
        ...
        if (COOKIE_PLACEHOLDER.equals(auth.getCookiePassword())) {
            stillDefault.add("EVALS_AUTH_COOKIE_PASSWORD");
        }
        if (SEALING_PLACEHOLDER.equals(evals.getSecretKey())) {
            stillDefault.add("EVALS_SECRET_KEY");
        }
        if (LAUNCHER_PLACEHOLDER.equals(rca.getAgentic().getLauncherApiKey())) {
            stillDefault.add("EVALS_OBSERVER_AGENTIC_LAUNCHER_API_KEY");
        }
        if (stillDefault.isEmpty()) {
            return;
        }

        String domain = evals.getSiteDomain();
