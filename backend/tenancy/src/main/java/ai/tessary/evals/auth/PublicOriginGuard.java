 ...
 * ({@code SITE_DOMAIN}, {@code EVALS_AUTH_FRONTEND_URL}, {@code WORKOS_REDIRECT_URI}) and nothing
...
public class PublicOriginGuard {
    ...
    private static final String[] MCP_ORIGIN_KEYS = {
        "evals.rca.agentic.mcp-base-url", "evals.classifier.triage-mcp-base-url"
    };

    private static final String[] MCP_ORIGIN_ENV = {
        "EVALS_RCA_AGENTIC_MCP_BASE_URL", "EVALS_CLASSIFIER_TRIAGE_MCP_BASE_URL"
    };

    private final EvalsProperties evals;
    private final AuthProperties auth;
    private final WorkOsProperties workos;
    private final Environment env;

    public PublicOriginGuard(EvalsProperties evals, AuthProperties auth, WorkOsProperties workos, Environment env) {
        this.evals = evals;
        this.auth = auth;
        this.workos = workos;
        this.env = env;
    ...
    void verify() {
        ...
        String raw = evals.getSiteDomain() == null ? "" : evals.getSiteDomain().trim();
        ...
        String mode =
                evals.getTlsMode() == null ? "acme" : evals.getTlsMode().trim().toLowerCase(Locale.ROOT);
        if (!TLS_MODES.contains(mode)) {
            throw new IllegalStateException(
                    "Refusing to start: TLS_MODE must be one of acme, owncert or upstream, not '" + evals.getTlsMode()
                            + "'.");
        }
        if ("acme".equals(mode)
                && (evals.getAcmeEmail() == null || evals.getAcmeEmail().isBlank())) {
        ...
        String frontend = reconcile("EVALS_AUTH_FRONTEND_URL", auth.getFrontendUrl(), domain, origin);
        ...
        log.info(
                "public origin {} from SITE_DOMAIN={} (TLS_MODE={}, EVALS_AUTH_FRONTEND_URL={}, WORKOS_REDIRECT_URI={})",
                origin,
                domain,
                mode,
                frontend,
