...
import static ai.tessary.evals.crypto.CryptoConstants.ALG;
import static ai.tessary.evals.crypto.CryptoConstants.IV_LEN;
import static ai.tessary.evals.crypto.CryptoConstants.TAG_BITS;
import static ai.tessary.evals.crypto.CryptoConstants.XFORM;
...
/**
 * AES-256-GCM sealing for the session cookie. Mirrors {@link ai.tessary.evals.crypto.SecretBox}
 * but with its own key ({@code EVALS_AUTH_COOKIE_PASSWORD}) so a leaked ingestion-credential
...
public final class SessionCipher {
    ...
    public SessionCipher(AuthProperties props, ObjectMapper mapper) {
        ...
        try {
            ...
            throw new IllegalStateException("EVALS_AUTH_COOKIE_PASSWORD must be base64", e);
        }
        if (raw.length != 32) {
            throw new IllegalStateException(
                    "EVALS_AUTH_COOKIE_PASSWORD must decode to 32 bytes (AES-256); got " + raw.length);
        }
        this.key = raw;
    }
    ...
    private byte[] requireKey() {
        if (key == null) {
            throw new IllegalStateException(
                    "EVALS_AUTH_COOKIE_PASSWORD is not configured; set it to base64 of 32 random bytes");
        }
        return key;
    }
}
