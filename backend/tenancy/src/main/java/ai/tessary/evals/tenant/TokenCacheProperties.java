 ...
 * {@code evals.auth.token-cache.*}, because that is where an operator will look for it.
 *
 ...
 */
@Component
@ConfigurationProperties(prefix = "evals.auth.token-cache")
public class TokenCacheProperties {
    ...
    public void setMaxEntries(int v) {
        if (v < 1) {
            throw new IllegalArgumentException("evals.auth.token-cache.max-entries must be at least 1, not " + v);
        }
        this.maxEntries = v;
    }
    ...
    public void setMaxRejections(int v) {
        if (v < 1) {
            throw new IllegalArgumentException("evals.auth.token-cache.max-rejections must be at least 1, not " + v);
        }
