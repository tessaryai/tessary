...
# Proves the self-hosted default deployment reaches exactly one outbound destination,
# home.tessary.ai, and that TESSARY_TELEMETRY_ENABLED=false actually suppresses it rather than
# merely documenting that it should.
#
# Separate from check-open-boot-selfhost.sh, which boots docker-compose.yml and proves the stack
# comes up behind the auth guard with no denied credential present, a static assertion over
# rendered config and container env. It has no opinion on what the stack actually dials once it is
# up: adding a packet capture to an already-dense boot check would bury one failure mode inside
# another script's unrelated exit code. This file's only job is what the network actually saw
# leave the bridge.
#
# Lives under scripts/lib/, not as scripts/check-egress-capture.sh: scripts/check.sh's manifest
...
# run yet.
#
# This is the compose-network vantage only, one of what could be several (host-daemon,
# served-page). It captures and diffs egress at the compose network's own bridge; a broader
# multi-vantage harness should call this script's capture-and-diff logic as one of its vantages
# rather than re-deriving DNS-capture code a second time.
#
# Never run agent-side: it needs Docker to build and boot a real stack, plus tcpdump and enough
# host privilege to sniff a bridge interface, which an agent sandbox has neither of and should not
# be granted. Run by a human (`task egress:capture`), who reads the destinations it printed and
# records the result (date, destinations seen, kill-switch outcome) as evidence for the claim this
# script exists to support. This script does not write that record itself, and no separate dated
# sign-off artifact should exist for it.
#
# What each pass proves. Two passes, same export, same compose file, only
# TESSARY_TELEMETRY_ENABLED flipped between them:
#   pass 1 (TESSARY_TELEMETRY_ENABLED=true, the shipped default): boot the stack, wait for it to
#     come up, let one heartbeat window elapse (TelemetryHeartbeat's own initialDelay is 5s plus up
#     to 25s of jitter, see backend/surfaces/.../telemetry/TelemetryHeartbeat.java, so this pass
#     waits comfortably past that), then read back every DNS query name and every outbound TCP SYN
#     observed on the compose network's bridge interface for the whole window. Expect exactly one
#     distinct destination hostname: home.tessary.ai. Anything else is a real finding: it prints
#     every distinct destination it saw and fails the pass if the set is not exactly
#     {home.tessary.ai}.
#   pass 2 (TESSARY_TELEMETRY_ENABLED=false): same boot, same wait, same capture window. Expect
#     zero destinations of any kind, since the contract's own wording is "zero outbound calls
#     including DNS resolution", so even a DNS-only, connection-refused attempt at home.tessary.ai
#     is a failure of this pass, not a partial pass.
#
...
# (not a container), and on most hosts needs to run as root or with CAP_NET_RAW/CAP_NET_ADMIN to
# open that interface in promiscuous capture mode, the same privilege class
...
# The one destination the shipped default is allowed to reach, per the published table this pass
# is proving (docs/self-hosting/configuration.mdx's Telemetry section). Kept as a variable, not
# inlined into every check below, so a future contract-version bump that adds a genuinely new
...
capture_pass() {
    ...
    # value", the destination set, goes out the same way: through the global CAPTURE_DESTINATIONS,
    # set right before every return, not through stdout.
    CAPTURE_DESTINATIONS=""
...
# doesn't), which is the assertion this script actually needs; nobody is proposing a 24-hour CI job.
