...
# that forgets the filter still starts the stack, but with the 8 GB classify container that slim
# mode was asked to skip.
#
...
dev_up_services() {
    ...
    # the encoder weights (BAKE_EMBEDDERS bakes ~1.7 GB of them into the dev image) and whose
    # container is capped at 8 GB (mem_limit). The backend itself has no
    ...
    # model bake) even though its own name was filtered out. compile is also
    ...
    # `compile`, this grep stops matching it, and its `depends_on` drags the 8 GB classify
