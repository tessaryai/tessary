...
# The names a clean-room boot must never see resolve to a non-empty value — sourced by
# scripts/check-open-boot.sh, not executed on its own (no shebang needed, matches
...
#
#   WORKOS_API_KEY / WORKOS_CLIENT_ID / WORKOS_REDIRECT_URI — WorkOsProperties.java. WorkOS is live,
#     compiled code in backend/tenancy, selected over the default PasswordAuthProvider purely by
#     WorkOsProperties#isEnabled() (both WORKOS_API_KEY and WORKOS_CLIENT_ID set). A stray value
...
#   E2B_API_KEY — the sandbox-runner launcher's opt-in SANDBOX_BACKEND=e2b path (both compose files
#     pass it through; neither defaults the backend to e2b). Latent in the dev leg (the launcher
...
#   STRIPE_SECRET_KEY / STRIPE_API_KEY — Stripe's keys are deleted outright, not varied — a
...
#   MIXPANEL_* / LAUNCHDARKLY_* — fully absent from the tree: no SDK, no key, no config surface
#     (TelemetryProperties.java's own javadoc and application.yaml's feature-flag comment both say so
#     directly). Denying an env-var name that can no longer appear in the tree guards against
#     nothing.
#   ANTHROPIC_API_KEY / OPENAI_API_KEY — conspicuously not here. check-open-boot.sh's
#     authenticated-triage assertion deliberately introduces one of these as a legitimate, scoped,
#     self-hoster-style BYO provider credential (via ProviderCredentialController's REST upsert,
#     gated on Capability.BYO_PROVIDER_KEYS, on by default) to exercise the model-graded classifier
#     gate end to end. Denying these two names would make the check fail itself on the one
#     credential the gate is meant to allow.
