...
namespaces|scripts/check-namespaces.sh|EXCLUDED:epic 6 clause 7's namespace ownership recheck (#1152): hits Docker Hub, GitHub and Hugging Face over the network; run via `task check:namespaces`, quarterly by namespace-recheck.yml and before every release by release.yml, never part of `task check`|EXCLUDED:same|declared here only so the completeness assertion can see it
