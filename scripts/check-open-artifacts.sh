...
# Every published image is enumerated and diffed against scripts/lib/open-artifact-denylist.txt:
# no unexpected class, no overlay jar or directory, no extra Liquibase lane, no trained model
# weight, and no route absent from this tree's own OpenAPI spec in the served frontend bundle.
# This reads the built artifact, never the source tree: a source-tree scrub proves the checkout
# carries no unwanted code and says nothing about a cache layer, a build argument, or a
# longer-lived branch that quietly re-includes something.
#
...
# With --images, only the roles given are diffed, useful when a caller hands in one image at a
...
if [ "$NEGATIVE" = 1 ]; then
    ...
    _has() { [ -n "$(_img "$1")" ]; }
    # The backend plant below stands in for content this build should never carry: a class under
    # a namespace this tree does not ship, plus a third-party flag-SDK jar it does not declare.
    if _has backend; then
        # A fat jar: one planted class under BOOT-INF/classes/ and one LaunchDarkly jar under BOOT-INF/lib/,
