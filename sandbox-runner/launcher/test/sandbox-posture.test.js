...
'use strict';
/*
 * #1020: the sandbox runners take a REQUIRED credential/network posture. Proves the three
 ...
 *      the launcher's own process env is full of provider keys (a leftover-env canary — #939 D4
 *      means agentEnvs() no longer reads any of these from process.env at all, but the posture
 ...
 *      NOTHING from the ambient process env — #939 D4's whole point is that agentEnvs() is now a
 *      pure function of its `credential` argument, never of process.env.
 *   3. Omitting the posture, or passing anything but the two sentinels, is a hard TypeError --
 *      the whole point of #1020 is that there is no silent default to full credentials.
 */
process.env.SANDBOX_BACKEND = 'local'; // keep require() side-effect free (no docker fail-fast, no listen)
// Leftover-env canaries: #939 D4 removed every credential env var agentEnvs() used to read, so
// these prove the posture/credential boundary holds on its OWN terms, not because nothing is set.
...
test('UNTRUSTED_POSTURE: local child env is exactly PATH + WORK_DIR, no credentials leak (#1020)', () => {
...
test('AGENT_POSTURE: local child env forwards the CREDENTIAL argument\'s own keys, not the ambient env canaries (#939 D4)', () => {
  ...
  // which send ONLY agentEnvs()'s output and never see process.env at all). What #939 D4 actually
  // changed is which VALUE wins: agentEnvs(credential)'s own value must win over (overwrite) the
