...
// ---- Datasets ----
/** A trace filter in request shape — the same field set the Explore page builds. */

// ---- Golden datasets ----


/** Snapshot the traces matching a filter into a new batch-runnable (span_sourced) dataset. */
...
export type ModelProvider =
  ...
  | "ANTHROPIC"
  | "OPENROUTER"
  // OLLAMA lived here until #939 D6 dropped it: it was the platform's sole AUTH_NONE,
  // platform-funded provider, and D6's maker filter (OpenAI, Anthropic, Google, Moonshot, Zhipu,
  // xAI) drops it — Meta is not a supported maker. Every provider requires an org credential now.
  | "MOONSHOT"
  | "BEDROCK"
  // #939: four new OpenAI-compat providers, all speaking the same Chat Completions wire as
  // OpenRouter/Moonshot against their own base URL (see PlatformCatalog.java). Google Vertex was
  // deferred — its service-account/ADC auth fits none of PlatformAuth's kinds — see the
  // open-core execution plan's divergence log.
  | "GEMINI"
  | "GLM"
  | "GROK"
  | "CUSTOM"
...
// "none" (AUTH_NONE) lived here until #939 D6 removed Ollama, its one user — every platform now
// authenticates with an org-supplied key.
...
// consumed by anything. Verified empirically against this repo's own `tsc --noEmit --strict`: an
// isolated alias in that exact shape produces zero diagnostics on a deliberately mismatched pair,
// and reproducing the drift in THIS file with only the type alias (no `const`) also produced zero
// diagnostics from this mechanism — every diagnostic came from Providers.tsx's own incidental use
// of the generated catalog types, which would go silent the moment that file stopped exercising
// every provider value. The `const` below forces evaluation: assigning `true` to a type that
...
// A (#994): one model's live per-MTok rate, read from the price book at request time — never hardcode
// a model's rate in the client, the book can move. Either field null means unpriced, not free.
export type ModelRateView = S["ModelRateView"];

// The "what's moving" trend types went with the CUSUM watcher (grader Layer 1): a sustained drop in a
// grader's pooled score was being read as a detection, and nothing could re-assert or close the cases
// it opened. `/trend/grader-stats` survives as the Graders index's counts and carries its own view type.

// ---- Grader verdicts ----

// ---- Grader drilldown — one grader's verdicts across every run/trace ----
/** One verdict's fields (the drill row + pill). Alias of the run-scoped verdict view. */
/** A verdict paired with the trace it judged — the cross-run grader-drilldown row. */

// ---- Dataset runs — ad hoc batch grading over a span_sourced dataset ----
// Same verdict shape as a grader-run escalation's verdicts.
...
/** Settings → Data retention (#1205): one row per data class the hourly sweep enforces. */
...
// ---- SOP conformance -----------------------------------------------------------------------
//
// The fit report DTOs (`ConformanceFitReport`, `ConformanceRuleFit`) and the `ConformanceSupply`
// enum used to live here, derived off the generated schema (#841's exception: the endpoint and
// its three schemas stayed in the open OpenAPI spec while the classifier that filled them was
// paid). #917/#944 replaced the one-spec rule with a per-edition spec, and #918 moved the
// controller and its DTOs into the paid classifier module along with the wire seam itself — so
// the open spec no longer carries `ConformanceFitReportView`/`ConformanceRuleFitView`, and these
// aliases would resolve to nothing. The paid frontend's rulebook rail now hand-types the response
// itself, mirroring `ConformanceDtos.java` directly rather than deriving from a schema only the
// paid edition's build can regenerate.

/** §9 — the two acted-on outcomes. The third outcome is doing nothing, which posts nothing. */
export type BehaviorResolutionAction = "expected" | "not_expected";

// ---- Behaviour drift: the fitted-epoch profile -------------------------------------------
//
// BehaviorProfile / BehaviorProfileState / BehaviorReadiness / BehaviorReadinessBlocker used to
// live here, one schema-derived and three hand-typed (#840's exception: the endpoint and its
// schema stayed in the open OpenAPI spec while the classifier filling them was paid). #917/#944
// replaced the one-spec rule with a per-edition spec, and #919 moved the route and its DTOs into
// the paid classifier module along with the wire seam itself — the open spec no longer carries
// `BehaviorProfileView`, and these aliases would resolve to nothing. The paid frontend's
// baselines rail now hand-types the response itself, mirroring the paid `BehaviorProfileView`
// Java record directly.

/**
 * §8 — one row per `(cause_kind, cause_key, workflow_key)`, never one per firing trace.
 * `workflowKey` is always `__global__` until conditioning returns (see FOLLOWUP-INTENT.md).
 */
/**
 ...
 * from a filtered one. `lane` is which Layer-2 lane this project's findings get ruled on; it replaced
 * a `triageAvailable` boolean that existed to say a repo-less project would never be
 * triaged at all, which is no longer a state that exists.
 */
