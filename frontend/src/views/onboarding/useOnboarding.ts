...
/*
 * The onboarding ladder, read once and shared (launch G3/G4/G6).
 *
...
export function useOnboarding({ poll = false }: { poll?: boolean } = {}): Onboarding {
  ...
  const query = useQuery({
     ...
     * surface — so moving Traces → Triage → Traces paid for it three times. The read is not cheap:
     * one of its five queries takes MIN/MAX over the project's whole observation table (migration
     * 0072 gives that an index; this keeps us from asking at all when we already know the answer).
     *
