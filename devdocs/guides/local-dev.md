...
| Run the encoder classifier service? | No (groundedness stays dormant; frustration still runs on your OpenRouter or TypeSafe key), Yes (8 GB container, gated weight download) | No | Yes |
...
So the product is whole in slim mode: the browser, every page, ingestion, triage, RCA. The one thing dormant is anything encoder-backed — classifier heads and SOP-conformance fits — which stays dormant until a full `task dev`. Frustration is not encoder-backed: it runs in slim mode once an OpenRouter or TypeSafe key is saved, so groundedness is the one classifier left dormant.
