...
`backend/core/src/main/resources/db/changelog/changes/` is applied, plus, where a private
paid overlay is present, its own changelog. Not exhaustive — see the
...
> live files. A second squash (2026-09) folded that baseline's own `0001`–`0016`
> follow-on chain back in (`0016` is the removal that dropped the eval/grading tables, which
> landed BEFORE the partition, so the fold absorbed it too) and,
> in the same commit, partitioned the schema: 15 paid-owned tables moved out to the paid
> overlay's own `P0000-paid-baseline.sql`,
...
- **There is no environment scope.** A project is the only scope below an org: `0016` dropped the `environment` table plus the `environment_id` column from `session`, `trace`, `span`, the (since-removed, `0017`) `embedding`, `api_key` and `metric_rollup`.
- **Typed subject grains** on subject-bearing tables: `session_id` (the session, or the trace when the producer sent no session) + optional `trace_id`/`span_id` + CHECK, with a `subject_kind` of `session`/`trace`/`span`. `0094` renamed the columns off v1's `context_id`/`observation_id` and migrated the discriminator to match — the all-databases reset is the sanctioned exception to the never-rename-a-persisted-string rule. Governance rows may still use `(subject_kind, subject_id)`.
...
erDiagram
    ...
    eval_case        ||--o{ finding : "case_id (ON DELETE SET NULL)"
...
| `provider_credential` | One sealed LLM credential per `(org, provider)`, shared by every project in the org — models the customer brings and pays for. `project_id` is a nullable historical pointer to whichever project's row won the org-scope dedup (`0020`), not part of the key. |
| `project_model_setting` | Which model each `ModelLane` (rca, triage, frustration) runs on. **A row is an override and nothing else** — it exists only because someone chose a model on the settings page. Project creation writes no rows (it cannot: a brand-new org holds no provider credential, so there is no model it could run), and there is no seeder SPI any more. **A lane with no row is the normal state**: it resolves through `llm/LanePriority` — **provider first, model second**. Each lane carries an ordered list of providers, and each provider carries every model that lane offers on it plus the one that is its default; the lane runs the default of the first provider the org holds a `provider_credential` for. No provider configured means no model at all, which is what the settings page says rather than naming a default the org cannot run. Adding a higher-priority provider later moves every lane nobody has pinned by hand. Provider order is per lane: RCA leads with Bedrock (Claude Sonnet 5), then Anthropic direct, OpenAI, Bedrock mantle, Gemini, xAI, Zhipu, OpenRouter, Moonshot, Custom. **TRIAGE is exactly RCA's list** — same providers, same order, same defaults — because there is no honest argument that an unattended ruling deserves a worse model than RCA would run on the same finding; a triage price ceiling used to give TRIAGE its own cheaper list and that is gone. Most providers also carry a smaller current-generation model, selectable on either lane but the default on neither, for a project that wants to point unattended TRIAGE at something cheaper on purpose. A model priced above its own provider's default is accepted; the settings page warns before saving one, reading the live price book rather than a fixed threshold, and does not warn when either price is missing. All ten providers the sandbox launcher supports appear on both lanes (`sandbox-runner/launcher/server.js` now has a mode for every one of them, including Anthropic direct, OpenRouter and Moonshot), so whichever single key an org holds, both lanes resolve. `CUSTOM` is last everywhere and is the one model whose price cannot be checked. Reset (`DELETE .../model-settings/{lane}`) **deletes the row**, returning the lane to that automatic answer. Writes are validated against `llm/BedrockModelProfile` + `llm/ModelCatalog` rather than by an FK: the model must exist, the model must be agentic for an `agent_vm` lane, it must be offered **on that lane**, and the org must hold a credential for its provider (`PROVIDER_NOT_CONFIGURED`). A stored row that stops satisfying any of those — a credential deleted, a tier withdrawn, a model removed — is left in the table and read as unset, so re-adding the credential brings the choice back. No lane sends a service tier or reasoning effort of ours, because the agent inside the microVM composes its own requests. So every write stores `service_tier` `standard` with `reasoning_effort` null, and a Bedrock row stored at any other tier reads as unset. `ck_project_model_setting_lane` was narrowed to five by `0007`, to three (`assistant`, `rca`, `triage`) by the pre-fold `0016` (folded into `0000-baseline.sql` at the 2026-09 partition), and to two (`rca`, `triage`) by `0018`, which deleted the in-app assistant feature and its `assistant` row; the open `0014` widened it to three with `frustration`, the one `decision_calls` lane. That lane takes only decision models (`TYPESAFE:jev-latest`, `OPENROUTER:typesafe/jev-latest`, TypeSafe first) and no other lane takes one, so its row is in effect a choice of gateway. Rows leave only with their project, by `ON DELETE CASCADE`, or by a reset. |
...
| `span_payload` | The heavy half of a span, split out: `input`, `output`, `attributes` jsonb, and `provided_usage` (an audit copy of the producer's raw usage object, never read for arithmetic). One row per span, written in the same transaction under the same `event_ts` guard, so the two can never disagree about which version they hold. It ages out ahead of `span`: a span whose payload has been purged stays fully functional on every list and rollup surface. `redactions` jsonb (`0003`) is what the `Credentials` redaction rule removed from this payload, as `[{rule, field, anchored, masked}]` and never any part of the credential; `masked` (forward-only, null on a row written before it existed) is the provider prefix plus the last 4 characters of the credential that was there, computed before the substitution — it is written in the same statement as the text it describes, so a newer version of the span replaces both, and it is how `secret_leak` names a leak, and which key leaked, whose credential is already gone. |
...
> `curation_entry` — every table the removal of the eval half left with no reader.
> `annotation` itself survived the teardown as a table, but no code writes it any more
> (`ProjectPurgeRepository` still deletes its rows with the project). A classifier files a
> **finding**, and a finding is judged by a triage ruling stamped on its own row.
...
| case | `eval_case`, `eval_case_event` | Triage. A case reads its findings through the reverse pointer, `finding.case_id` (`ON DELETE SET NULL`); it opens from one and any later positive on the same cause joins it, so "case → findings" is one-to-many now (`0011`) |

The chain `finding → evidence` is a real foreign key, and so is `finding → case` in the reverse
direction of the arrow above (`finding.case_id`, `0011`) — a case's findings, not a finding's case,
is the direction the product actually reads. The last hop, evidence → substrate, is deliberately
**not** an FK (see `finding_evidence`) — it is a pinned reference enforced by the retention predicate
instead.
...
| removed | replacement |
|---|---|
| `verdict` as the classifier detection store (`source='automatic'`, `timing='online'`, `mode`) | per-classifier `*_detection` tables (`0088`) + a query-time union built by `DetectionTableRegistry` (superseding the `0089` view); the three columns/values left the schema in `0095` |
| `signal_event_v` | `DetectionTableRegistry`'s query-time union, projecting the same column vocabulary verbatim |
...
| grader CUSUM watcher: trend-as-detector, the `grader_degradation` case source, `signal_trend_rollup` | nothing. It became Layer-2-only in `0089`/`0090`, and grading was then removed outright — `0016` drops the `grader*` tables and takes `grader_degradation` out of `eval_case_detector_check` |
...
| Table | Purpose |
|---|---|
| `classifier` | A project-scoped classifier definition (built-in or user-authored); `signal` until `0093` renamed it, key column included (`signal_key` → `classifier_key`). Also where a per-span classifier's **arming** lives: `config_json.arming` says how many detections in what window open or refresh a finding, translated off the threshold `alert_rule` rows by `0089`. An optional `confidence` member (`high`/`any`) sets the band that counts independently of `mode`, so a built-in can keep its low band listed (discovery) without counting it. Secret Leak ships armed at one high-band detection a day and files one finding per call site and pattern, counted on the window each span happened in rather than when the sweep wrote it; its `cause_key` is `<classifier_id>:<call_site_id>:<pattern>`. Its finding's `payload.confidence` is `high` once any counted detection was high-band, and stays `high`, so it outlives the detection rows retention removes. `paused_reason` / `paused_at` (`0015`, set together or not at all): a classifier that calls a provider on the org's own key stops on `provider_rejected` (401/403) or `no_provider` (no key for its lane); a paused sweep sends nothing and advances past what it skips, and the pause clears when a later sweep finds a usable key. |
| `classifier_model` | **Removed** (`0017`) with the centroid-over-embeddings user classifier — dropped alongside `embedding`/`embedding_space`. No successor; a user-authored classifier's state now lives entirely in `classifier.config_json`. |
...
| `annotation` | Survived the eval-half teardown (the queue/label/verdict tables it once fed did not). `annotator_kind`-tagged (`human`/`llm`/`consensus`) boolean/numeric/categorical examples keyed by `key`, at `subject_kind` grain (`session`/`trace`/`span`), with `of_finding_id` + `agrees` for a correction against a specific finding. Retained but unwritten: no code writes it now, and `ProjectPurgeRepository` deletes its rows with the project. |
...
Behaviour drift and its catalog entry are not in this tree, so nothing here writes the tables marked
**Paid.** below; they stay documented because the schema still declares them.
...
| `finding` | **One row per CAUSE, for every classifier** (`0085`). It replaced `behavior_finding` (shared by behaviour drift, metric drift and tool error) and `conformance_finding`, which were the same concept written twice with per-classifier vocabulary in the columns. `classifier_key` says who filed it — a column now, not a derivation from `cause_kind` — and the scope it fired against is the typed `(subject_kind, subject_id)` pair (`behavior_profile` / `metric_baseline` / `tool` / `conformance_rule`). Everything classifier-private lives in `payload` jsonb: the shift and its quantiles, the expectation test's statistics, `consecutive_confirmations`, and the native vocabulary the scoped key folds in (`cause_kind`, `workflow_key`, `native_cause_key`). **`cause_key` carries the scope a cause used to be unique within** — `<profile>:<kind>:<key>:<workflow>`, `<baseline>:<key>` (the baseline id carries environment scope; collapsing it merges staging into production), the tool-error key verbatim, `<rule>:<kind>` (0072 widened conformance's open-uniqueness to (rule, kind), so a fit-time baseline audit and a windowed drift test on one rule keep separate rows) — which is what lets ONE partial unique index, `ux_finding_live (project_id, classifier_key, cause_key) WHERE status IN ('open','blocked')`, replace the four scoped ones the old tables carried. **`blocked` is inside the live predicate deliberately**: a blocked row that left the index would conflict with nothing, the next firing would INSERT beside it, and the human verdict would be silently discarded (0033's bug class). `sample_count` is how much traffic the claim has been observed over in the classifier's own unit — traces, window samples, activations — and is **not** a label count: nothing labelled a trace on a distribution shift. `onset_at` is the CURRENT spell's start and moves only across an observed recovery, because `CaseLedger.isNewSpell` reads an advanced onset as a fresh spell. Both `onset_at` and `last_seen_at` are wall-clock for every classifier except metric drift and tool error. `recordShift` writes metric drift's closing window's own EVENT time into them [R11] — the same clock `metric_baseline.control_json` now keys and weighs on. `recordRecomputedCause` does the same for tool error: `last_seen_at` is the spell's last hourly bucket the detector folded, not the moment the sweep ran, and the 28-day replay itself anchors to the project's newest tool-call event rather than to wall-clock now, so a backfill whose traffic predates the anchor by months still falls inside the window. A confirmed-regression exclusion, a reopen, and liveness ordering all follow event time for these classifiers' findings; a replayed backfill therefore reads a historical regression as already over rather than as freshly firing, whichever of the two found it. Malformed Output shares tool error's `recordRecomputedRate` write but not its anchor — it has no traffic clock of its own, so it passes its sweep's wall clock through unchanged and stays wall-clock like every other classifier. The `triage_*` set (`triage_verdict` ∈ positive/negative, `triage_action` ∈ opened_case/closed, summary, citations, `triaged_at`) replaced the `adjudication_*` columns in `0009`, which DROPPED them and discarded every stored value: `expected|deviation` was a judgement about intent and `negative` is one about the detector, so no value read forward, and the drop also deleted the rows fail-open had written for runs that never happened. Triage rules on the CLAIM only — never on direction or impact, so a cost DROP is as `positive` as a rise — and the verdict fixes the action (`positive` → a case, `negative` → closed), which is why they are written in one statement under `finding_triage_paired_check`. A NULL verdict is not a ruling: a run that did not happen, never reached the evidence, or did not settle the question leaves it NULL and lets its job retry into the dead letter — `unclear` is gone (`0010`), which rewrote every stored `unclear` to `negative` and narrowed `finding_triage_verdict_check` to the two-way vocabulary. Closing does not touch `status` — the row stays live so its cause keeps firing against it, `recurrences_since_verdict` counts those firings, and the re-open rule sends it back through triage. **`payload` freezes once a ruling stands** (`triage_action` or `human_verdict_at` set): a firing after that still bumps `sample_count`, `last_seen_at` and `recurrences_since_verdict`, but the recorder's upsert leaves `payload` alone rather than re-pointing the claim a human or Layer 2 already read — the same gate `finding_evidence`'s MEMBER/BASELINE refs already carried. `reopenForTriage` nulls `triage_action`, which is what unfreezes both. `escalated_at` is the schedule-once marker both the button and the scheduler write, cleared by a re-open. **`0011` replaces all of that with one rule**: `status` is `open`/`closed` only (`graduated`/`allowlisted`/`blocked`/`resolved`/`absorbed` are gone), `ux_finding_live` narrows to `(project_id, classifier_key, cause_key) WHERE status='open' AND triage_verdict IS NULL`, and a ruling — machine or human, `recordTriage`/`recordHumanRuling` — leaves that index by construction, so the freeze `payload`'s CASE branches used to need is now structural: no upsert can conflict onto a ruled row at all, and the same cause firing again INSERTs a fresh open row rather than reopening a settled one. `positive` keeps the row open for as long as the case it opened or joined (`case_id`, the reverse of the old `eval_case.finding_id`) stays unresolved; `negative` closes it outright. `recurrences_since_verdict` and the reopen machinery it drove (`TriageAutoEscalator.reopenRecurring`) are deleted with no replacement — a finding that disagrees with the traffic again is a new finding, triaged like any other. |
| `finding_evidence` | **The substrate a finding's claim is based on — references, never copies** (`0085`). Multiplicity is row count, grain is which column is set (`session_id` alone / `trace_id` alone / `trace_id` + `span_id`, since span identity under substrate v2 is the composite `(project_id, trace_id, id)` and a bare span id resolves to nothing), and set membership is `role` ∈ exemplar/member/baseline/witness/changepoint. **Append-only, with one narrow exception**: metric drift re-points `member`/`baseline` at the window that just fired on every close (`FindingEvidenceRepository#replace`), but only while the finding is unruled — a ruling freezes it (spec §5). It replaced `behavior_finding.exemplar_trace_id` — literally one trace per finding — which is why a rate shift's witnesses had to live inside a JSON blob and a conformance rule's ranked violations inside another. **Deliberately no FK into `trace`/`span`/`session`**: a NO ACTION FK would block retention from ageing out substrate referenced by a CLOSED finding, and v2's own dependent-table pattern is loose project-scoped producer columns. The pin lives in the retention predicate instead, scoped to LIVE findings and unresolved cases. **The writer does not cap.** `record` writes the whole list it is given, because `member` and `baseline` promise enumeration; a classifier filing `witness` rows, which promise none, caps its own through `recordUpTo` (Secret Leak and Malformed Output keep 50 per finding), so a cause that keeps firing does not pin an ever-growing slice of retention. Frustration caps neither side, because its witnesses are a rate's numerator: like Tool Error it writes every session its rate counted as `member` and every frustrated one as a `session` witness followed by a `trace` witness for the turn that fired. `ux_finding_evidence_ref` makes a repeated reference a no-op, which is what lets the tool-error recompute write its evidence on every pass. |
...
> Where the behaviour-drift module is installed (not in this tree), its sweeps ride the ordinary `job`
> `kind='classifier'` row of their classifier (the cursor
...
> A detection is a row in its classifier's OWN table (migration `0088`). The six tables' column
> vocabulary is stitched for the query API and the events feed
...
> `created_at` is `timestamptz`; a NULL `confidence` means HIGH. `frustration_detection` is created by
> the open changelog (`0016`, `IF NOT EXISTS` over the paid overlay's copy) with a nullable
> `subject_span_id`, `subject_session_id` holding the conversation key `COALESCE(thread_id,
> session_id)`, one row per flagged turn, and `cleared_at` (a human clear of the conversation's flag,
> NULL while it stands) read through the partial index `ix_frustration_detection_uncleared`
...
> clears every session cited by a session witness on any of the case's findings, which is every
> frustrated session of the spell. The open
...
> [`classifier-extension-interface.md`](classifier-extension-interface.md) §2/§8. A table with no registered `DetectionTable`
...
| Table | Purpose |
|---|---|
| `metric_baseline` | One window state per `(project, classifier, measure, bucket_kind, bucket_key)` — `ux_metric_baseline_scope`. It used to carry an `environment_id`, wrapped in a `COALESCE(..., '')` because NULL is distinct from itself in a unique index; `0016` dropped the column and recreated the index without the wrapper. Rows that collided under the narrower key were **deleted rather than merged**: a baseline is a population statistic (see `MetricBaselineRow`), and two populations do not add. `measure` ∈ `turn_duration`/`tool_duration`/`cost`/`tok_input`/`tok_output`/`tok_cache_read`/`tok_cache_write` and `bucket_kind` ∈ `call_site`/`tool` are **persisted strings, never renamed**. The bucket is the entry-point call site (or an `ActionSymbol` `kind:name` for tools) and deliberately **excludes model and project version**: anything you want to detect a change in must not be in the key, or a silent reroute to a model costing 30× more redefines normal and the detector goes quiet through the exact event it exists to catch. `state` ∈ `learning`/`armed`; a thin bucket stays `learning` and keeps accumulating rather than being skipped, so a rare tool is watched on a slower clock. Two references over `log(value)`, because one is insufficient in one direction each: `pinned_sketch_json` (pinned after the last deploy, with `pinned_at`/`pinned_by_version_id`) catches cumulative creep but screams forever once something legitimately changed, and `control_json` (0056) catches sudden breaks but never notices a slow boil. `current_sketch_json` is what both are compared against. `control_json` is the **rolling control**: one slot per UTC EVENT day over 21 days, each an exact merge of the windows that closed in it, weighted `2^(-age/7)` per sample — age measured back from the judged window's own event day, never wall-clock now [R11] — and reported to the detector at Kish's effective sample size. Retention is anchored on the newest day the ring holds rather than on wall-clock now, so an out-of-order import can never evict a day more recent than the one it is importing; a live install's event clock and wall clock stay within seconds of each other, so this only shows up on a backfill or a history import. It replaced `prev_sketch_json` — one arbitrary closed window was as noisy as the window it judged, took whatever shape the clock gave it, and forgot a step change immediately since the next window's previous IS the new level. The days a **confirmed** regression ran through are excluded from it, decided on every read rather than when the day was folded, so a Layer-2 verdict landing hours after the window closed acts retroactively and the shift under investigation never becomes the bar it is judged against. `prev_sketch_json` and `prev_workload_json` were dropped by `0092`, having been unwritten since `0056`; `prev_tokens_json` is the same retired slot and still stands. The pinned slot and each control day carry two sidecar blobs beside the sketch, moving in lockstep. `*_workload_json` is what the **user** asked for over the same samples (prompt size, message length, thread depth), never what the agent chose to do — an agent decomposing "what's my balance" into eleven tool calls IS the bug, so span count is neither a key nor a covariate. `*_tokens_json` (0043) is what a **cost** window's dollars were made of — the four token buckets and the cache-read share — and it is a separate blob precisely because the workload one is the ask and output tokens are the answer; folding them together would break the flat-inputs-beside-moved-outputs argument in the one block that exists to make it. It is null on every duration baseline. All of them are persisted rather than recomputed because a reference window is history: by the time a finding is written, the traffic the pinned sketch summarizes is weeks past and no query can re-derive what its input, or its cache-read share, looked like then. |
| `frustration_assessment` | One row per user turn the Frustration classifier sent to its decision model, flagged or not (`0017`): trace, span, `conversation_id` (`COALESCE(thread_id, session_id)`), the turn root's `call_site_id`, `turn_started_at` (the trace's start), `frustrated` (score above the threshold when scored), `scorer_version` (a hash of the question, its instruction and the threshold), provider, the model the provider echoed, the exact `request` (state and questions) and `response` bodies as jsonb, input tokens, the booked `cost_usd` and `latency_ms`. Unique on `(project_id, classifier_id, trace_id, scorer_version)`, so a re-sent page writes each turn once and a changed question writes new rows beside the old. No per-probability columns: the answer lives whole in `response`. A fact table: the scorer inserts, nothing updates a row except retention, which nulls `request` when the turn ages past the traces TTL (same pin as `span_payload`), and every reader lists columns rather than selecting `*`. Joins the substrate by key and never edits it. |
| `frustration_state` | The Frustration rate test's CUSUM state, one row per `(project, call_site_id)` (`0018`). Columns match `malformed_output_state` one for one, key apart, because `ToolErrorStateRepository` reads and writes both. `baseline_calls`/`baseline_failures` hold the learned reference in conversations and frustrated conversations (the first 200 scored under the current scorer, then frozen). The replay rebuilds the accumulator every pass from `frustration_assessment` grouped by `conversation_id` (a conversation is a trial on the call site of its first scored turn, bucketed in that turn's UTC hour, and a failure while it holds an uncleared `frustration_detection`); the row keeps the reference, the last accumulator for the Tuning view, and `reset_*`. A reset (a resolve, or `tuning changed` when `state_epoch` no longer matches the scorer version and dials) clears the reference too, and the replay skips every hour before `reset_at`. Its own table so no other classifier's ruling can touch it. |
| `groundedness_assessment` | One row per answer the Groundedness model scored, flagged or not (`0025`): trace, span, session, the span's `call_site_id` (`''` when unattributed), `unsupported` (the strongest sentence's P(unsupported)), `flagged` (at or above the one threshold), `scorer_version` (a hash of the model revision, the threshold and the input layout), `observation_started_at` (the span's start, the clock the replay buckets on) and `scored_at`. Unique on `(project_id, classifier_id, subject_trace_id, subject_span_id, scorer_version)`, so a rewound sweep counts each answer once. Answers the detector skips write nothing: they are not trials. Insert only. `groundedness_detection` holds the flagged answers' sentences; this table is the denominator it cannot give, and its newest `scored_at` is the "No scores since" time. |
| `groundedness_state` | The Groundedness rate test's CUSUM state, one row per `(project, call_site_id)` (`0025`). Columns match `frustration_state` one for one, because `ToolErrorStateRepository` reads and writes both. A trace is one trial on its call site, bucketed in the UTC hour its first scored answer started, and fails when any of its flagged answers still has an uncleared `groundedness_detection` (`cleared_at` is set when a person resolves the case as a false alarm). The engine judges from 200 traces and folds whole buckets into `baseline_calls`/`baseline_failures` until 1,000, then freezes the reference. |
| `malformed_output_state` | Malformed Output's CUSUM state, one row per `(project, call_site_id)` (`0004`). The same accumulator `tool_error_state` carries per tool, because Malformed Output replays each call site's hourly checked/failed tallies through tool_error's engine (`ToolErrorTrend` over `ToolErrorDetector`); columns match `tool_error_state` apart from the key, since `ToolErrorStateRepository` reads and writes both. Its own table so the tool-error sweep never loads a call site as a tool. Unlike tool_error it rebuilds the accumulator every pass against the frozen reference instead of resuming from `watermark_bucket`: a backfill or a schema-triggered rewind re-checks hours already tallied, and a resume would keep the first, partial count of each. The `pending_pin_*`/`reset_*` columns have no writer yet. |
...
| `git_webhook_delivery` | Inbound webhook idempotency / delivery log. Unused: the webhook ingress is gone and no code reads or writes it. |
...
> risk forecast on classifier detections is future work.

### Cases (Triage)
| Table | Purpose |
|---|---|
| `eval_case` | One detection that crossed its detector's bar, in the one shape that reaches a human. Cause-neutral: `detector` ∈ behavior_drift/classifier/metric_drift/tool_error/**sop_conformance** (`0063`, narrowed by `0016`) is the only field that says what noticed. Identity is `(project, detector, subject_kind, subject_id, metric)`; `ux_eval_case_live` enforces one non-resolved case per key. `finding_id` (`0085`, replacing `source_ref`) is a real FK to `finding` with **ON DELETE RESTRICT** — a case is what a human is looking at, so deleting the finding under it would leave a triage row that can no longer say what it is about, and the delete fails instead. It is what the case page reads its ruling, citations and exemplars from, in ONE lookup: the old column branched on `detector` to decide which finding table to ask, and asking the wrong one returned nothing silently. A forward CHECK (`ck_eval_case_finding_forward`) is `finding_id IS NOT NULL OR state = 'resolved'` since `0095`: a LIVE case points at a finding, and a resolved one may not. It carried two escape arms before that — a detector list (`grader_degradation`/`classifier`, whose spells were computed rather than stored) and `opened_at < '2026-08-13T00:00:00Z'` — which `0089`/`0090` could not remove, because retiring a source *force-resolves* its live cases, and that sets `state` without backfilling `finding_id` while the CHECK had no state arm. `0095` gave it one, after force-resolving every remaining finding-less live case (no stub findings — `0086` refused that in as many words). `eval_case_detector_check` narrowed with it, to exactly the detectors `CaseRow.Detector` declares — six then, five since `0016`, seven since `0007`, eight since `0019` (open numbering): `behavior_drift`/`classifier`/`metric_drift`/`tool_error`/`sop_conformance`/**`secret_leak`**/**`malformed_output`**/**`frustration`**. `grader_degradation` left when grading was removed; `0016` deletes its surviving cases (and their `eval_case_event` rows) before re-adding the CHECK, because no detector remains that could re-assert or close one. `0089` had added `secret_leak`/`malformed_output`/`groundedness`/`frustration`/`regex` as detectors and `0016` folded all five back under `classifier`, every per-span case filing generically. `0007` reopens exactly two of the five under their own names — `SecretLeakCaseSource` and `MalformedOutputCaseSource` — because a generic `classifier` case can't carry what their own finding pages need: a high-confidence secret leak opens with no triage gate at all, and never recovers on its own: its source lists every live high-confidence finding with no recency window or cap, since a credential that stopped leaking still needs rotating, and resolving the case resolves the finding (`0009` backfills the `confidence` its finding now records), and a malformed-output case wants its own subject (`call_site`) and measured rate rather than the classifier row. `groundedness`/`regex` stay under `classifier`; nothing about them needed that. `0019` reopens `frustration` under its own name for `FrustrationCaseSource`: a call site whose share of frustrated conversations rose above its learned rate (subject `call_site`, measure `frustration_rate`), opened with no triage gate, since each spell's finding is ruled positive when `FrustrationRateService` files it. `resolution` ∈ recovered/human/**absorbed** (`0052`) — `absorbed` means the detector's reference moved to include the shift, so the same level will not open a second case, which `human` does not. **`0011` drops `finding_id`** (and `ck_eval_case_finding_forward`/`ix_eval_case_finding_live` with it): a case's findings are read through the reverse pointer, `finding.case_id`, instead of a forward one the case row carried and had to keep in step on every refresh. `locked_at` is new — set once a person presses Run RCA on the case, so the cause's next positive opens a NEW case rather than joining one an agent is mid-analysis on — and `ux_eval_case_live` narrows with it to `state <> 'resolved' AND locked_at IS NULL`, so a key may have any number of locked cases beside its one live, unlocked one. `disposition` (`0020`, nullable, CHECK `fixed`/`false_alarm`) is what a person said a resolved frustration case was, and is refused on any other detector's case. Either value restarts the call site's `frustration_state` and clears its learned reference (`resetAndRelearn`); `false_alarm` also sets `cleared_at` on the `frustration_detection` rows of every conversation the case cites. Both land in the resolve's transaction, before the case's findings close, and the `resolved` trail line carries `{disposition, conversations_cleared}` in its `detail`. A frustration case resolved with no disposition restarts the call site and clears nothing. |
| `eval_case_event` | The activity trail. `kind` ∈ opened/reopened/escalated/rca_requested/rca_completed/recovered/resolved/muted/unmuted/absorbed/**recurred** (`0052`, `0011`). `reopened` and `recovered` are history only since `0011`: nothing writes either any more (no periodic sweep to notice a recovery, and a closed case is final), but old rows keep them so an old trail still reads. `recurred` is `0011`'s replacement for the live half of `reopened` — a second finding joining a still-open case, not a resolved one coming back. Written in the same transaction as the state change it describes — a case with no trail entry is one a reader cannot account for. |
| `rca_report` | One immutable RCA run about one finding: the subject, window and value columns are a snapshot taken at the press, and `verdict`, `summary`, `ruled_out` (the checklist), `hypotheses`, `detailed_report` and `repo_available` are stamped once at completion. `report_kind` (`0021`, NOT NULL, default `metric_movement`) says which question the run answers: `metric_movement` asks what change moved a number, `frustration_causes` asks what the agent did that frustrated the users of one call site. A frustration report has no baseline side, so it writes no `hypotheses` and no `serving_model` check; it writes `causes` (`0021`, jsonb, NULL on every other kind), ranked by sessions affected, each citing frustrated sessions and flagged turns from the finding's own evidence, and its `current_value`/`prior_value` are the rate since onset and the learned rate. `rca_report_verdict_check` gains `causes_identified` and `no_cause_found`, the two verdicts only that kind reaches. |

### Alerts (consolidated)
| Table | Purpose |
|---|---|
| `alert_rule` | The ONE unified alert config: `rule_type` ∈ threshold/digest/brief/**case_opened**/anomaly/trace/log/exception. Threshold/anomaly are per-classifier (`classifier_id`, `basis`, `threshold`, `window_seconds`); digest/brief are per-project scheduled roll-ups (`digest_cron`/`brief_cron`); `case_opened` is one per project and carries cadence + quiet hours in `attributes` (`AlertPolicy`) with `last_evaluated_at` as its anchor. `enabled` + `snoozed_until` are the two suppression controls. **`threshold` is a retired rule type with live history**: `0089` translated every threshold rule's parameters onto its classifier's `config_json.arming` and then **disabled** the rules rather than deleting them (parameters logged, rules still inspectable), because a raw-detection-volume notification is exactly the door the pipeline closed — the per-span classifiers file findings now, and `case_opened` is the only path from a classifier to a human. `anomaly`, `trace`, `log` and `exception` are schema-only: nothing creates or evaluates them. The digest/brief activity roll-up reads `DetectionTableRegistry`'s query-time union. |
...
| Table | Purpose |
|---|---|
| `job` | Unified work queue by `kind` ∈ `classifier`/`pull`/`usage_rollup`/`rca`/`behavior_profile`/`triage`/`sop_compile`/`project_delete`. `0016` narrowed `job_kind_check` when `observer`, `synth`, `grader_run` and `dataset_run` lost their workers; `0017` narrowed it again when `embedding` lost its. Leased kinds share SKIP-LOCKED claim; `usage_rollup` has its own claim SQL. `page_retries` (`0015`) counts consecutive holds of the page past a classifier job's cursor: a page whose provider calls mostly failed is held and re-sent, then skipped after `tessary.frustration.page-retries`; any cursor move resets it. |

> **Vector store — removed (`0017`).** `embedding_space` (the per-project space registry)
