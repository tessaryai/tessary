...
| `retention/` | `substrate` | Retention enforcement: `RetentionResolver` (platform default TTL, overridden by a `retention_policy` row, clamped to the `RetentionCeiling` seam another build may supply), `RetentionSweeper` (the bounded hourly delete), and `RetentionController` (Settings → Data retention: reads the resolver, writes the override rows; `RETENTION_MANAGE` to write). One resolver, so the sweep and the customer-facing answer cannot differ. | [ingest-runbook.md](../guides/ingest-runbook.md) |
