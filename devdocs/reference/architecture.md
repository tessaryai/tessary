...
| `classifier/` | `analysis` | async classifier-detection engine (Layer 1), plus the Layer-2 triage lane that rules on what it files. `classifier/frustration/` holds the Frustration classifier's decision-model detector (`JevFrustrationDetector`, a `PagedDetector` that scores a whole page on the org's key and writes its own rows), the turn builder it sends, and the `frustration_assessment` ledger. | [The three analysis layers](#the-three-analysis-layers-and-the-firewall-between-two-of-them) |
