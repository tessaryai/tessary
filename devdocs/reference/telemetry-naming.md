...
3. **Entity identity goes in attributes / observation metadata**, not the name.
   ...
   `evals.project.id` / `langfuse.observation.metadata.project_id`.
...
| Key | Where | Purpose |
|---|---|---|
| `evals.triage.finding_id` | observation metadata | The finding a Layer-2 ruling is about, and the ledger subject its spend books against |
| `evals.project.id` / `project_id` | span attribute; `langfuse.observation.metadata.project_id` (LlmCaller) or `langfuse.trace.metadata.project_id` (agentic sandbox roots) | Tenant join key |
...
[`LogContext`](../../backend/shared/src/main/java/ai/tessary/evals/open/obs/LogContext.java);
